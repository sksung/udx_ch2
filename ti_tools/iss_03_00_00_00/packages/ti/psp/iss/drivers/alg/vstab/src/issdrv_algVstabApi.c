...
int VIDEO_vsTskCreate()
{
    ...
    gVsStream.vsCreatePrm.totalFrameWidth = pObj->createArgs.scParams[0].inWidth; 		//1920;
    gVsStream.vsCreatePrm.totalFrameHeight = pObj->createArgs.scParams[0].inHeight; 	//1080;
...
void video_bsc_handle()
{
    ...
    {
        if (tempStreamBuf[0].ptmpFrame->addr[0][0] != NULL)
        {
			...
			nonStabilizedWidth0  = (pObj->createArgs.scParams[0].outWidth * VSTAB_SCALE_NUMERATOR)/VSTAB_SCALE_DENOMINATOR;
			nonStabilizedHeight0 = (pObj->createArgs.scParams[0].outHeight * VSTAB_SCALE_NUMERATOR)/VSTAB_SCALE_DENOMINATOR;
			nonStabilizedWidth1  = (pObj->createArgs.scParams[1].outWidth * VSTAB_SCALE_NUMERATOR)/VSTAB_SCALE_DENOMINATOR;
			nonStabilizedHeight1 = (pObj->createArgs.scParams[1].outHeight * VSTAB_SCALE_NUMERATOR)/VSTAB_SCALE_DENOMINATOR;
            ...
            offset  = (startY1 * nonStabilizedWidth1 + startX1);
            offset1 = (startY1 * (nonStabilizedWidth1 >> 1) + startX1);

            offset2 = (startY0 * nonStabilizedWidth0 + startX0);
            offset3 = (startY0 * (nonStabilizedWidth0 >> 1) + startX0);
			...
			{
				tempStreamBuf[0].ptmpFrame->addr[0][0] = (Ptr) (((Int32) tempStreamBuf[0].ptmpFrame->addr[1][0] + (offset2 << 1)) & 0xFFFFFFE0);
			}
            ...
            if (tempStreamBuf[1].ptmpFrame != NULL)
            {
				...
				{
				    tempStreamBuf[1].ptmpFrame->addr[0][0] = (Ptr) (((Int32) tempStreamBuf[1].ptmpFrame->addr[1][0] + (offset << 1)) & 0xFFFFFFE0);
				}
            }
#ifdef ISS_DEBUG_RT
            Vps_rprintf
        ...
        {
            Vps_rprintf("BSC: No buffer available to stream ID: %d", 0);
        }
    }
}
