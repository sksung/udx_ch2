 ...
 *  ==================================================================
 */
ISP_RETURN rszASetOutConfig(int inWidth, int inHeight, int outWidth, int outHeight)
{
    int vertReszRatio = 0;
    int horzReszRatio = 0;
 ...
 *  ==================================================================
 */
ISP_RETURN rszBSetOutConfig(int inWidth, int inHeight, int outWidth, int outHeight)
{
    int vertReszRatio = 0;
    int horzReszRatio = 0;

    //# set in rszASetOutConfig
    //rsz_reg->SRC_VSZ = inHeight - 1;
    //rsz_reg->SRC_HSZ = inWidth - 1;
...
//# for dynamic change resolution - phoong
ISP_RETURN rszBSetCropOutConfig(int sx, int sy, int wi, int he)
{
    int ReszRatio = 0;

	//# comment because set when create
	//rsz_reg->SRC_VSZ = 1080;
    //rsz_reg->SRC_HSZ = 1920;
    //rszB_reg->RZB_O_VSZ = 480;
    //rszB_reg->RZB_O_HSZ = 720;

    rszB_reg->RZB_I_VPS = sy;
    rszB_reg->RZB_I_HPS = sx;

	//ReszRatio = (he * 256) / 480;
	ReszRatio = (he * 256) / (rszB_reg->RZB_O_VSZ+1);
    if (ReszRatio < 16)
        ReszRatio = 16;
    if (ReszRatio > 4096)
        ReszRatio = 4096;
    rszB_reg->RZB_V_DIF = ReszRatio;

    //ReszRatio = (wi * 256) / 720;
    ReszRatio = (wi * 256) / (rszB_reg->RZB_O_HSZ+1);
    if (ReszRatio < 16)
        ReszRatio = 16;
    if (ReszRatio > 4096)
        ReszRatio = 4096;
    rszB_reg->RZB_H_DIF = ReszRatio;
