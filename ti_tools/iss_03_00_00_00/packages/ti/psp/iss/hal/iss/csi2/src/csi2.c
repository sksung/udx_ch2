...
CSI2_RETURN csi2_reset(CSI2_DEVICE_T device_num, uint32 reset_complexio)
{
    ...
    csi2_regs = (device_num == CSI2_DEVICE_A) ? (csi2A_regs) : (csi2B_regs);
...
CSI2_RETURN csi2_open()
{
    ...
    csi2B_regs = (csi2_regs_ovly) CSI2B_BASE_ADDRESS;
...
CSI2_RETURN csi2_config_phy(CSI2_DEVICE_T device_num, csi2_phy_cfg_t * cfg)
{
    ...
    // // base_add = (uint32
    // )((device_num==CSI2_DEVICE_A)?(csi2A_regs):(csi2B_regs)); 
                                    ...
                                    CSI2_CONTROL_FEATURE_BIT_T on_off)
{
    ...
    base_add =
        (uint32) (device_num ==
                  CSI2_DEVICE_A) ? ((uint32) csi2A_regs) : ((uint32)
                                                            csi2B_regs);
                                          ...
                                          csi2_ctx_transcode_cfg_t * cfg)
{
    ...
    base_add =
        (uint32) ((device_num == CSI2_DEVICE_A) ? (csi2A_regs) : (csi2B_regs));
...
CSI2_RETURN csi2_config(CSI2_DEVICE_T device_num, csi2_cfg_t * aConfig)
{
    ...
    /* Configuration complex i/o settings of the CSI2 receiver.  Here, both
     * CSI2_IO_1 and CSI2_IO_2 are configured. In practice, one might like to 
     * configure and enable just one. Other can be commented out.  */
