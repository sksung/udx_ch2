...
/** \brief Value to use, when it is needed to apply device specific default
  *        setting value
*/
#define VPS_VIDEO_DECODER_DEFAULT                 (0)

/** \brief Value to use, when it is needed to NOT change a device specific
  *        setting value
*/
#define VPS_VIDEO_DECODER_NO_CHANGE               (-1)
...
#define IOCTL_VPS_VIDEO_DECODER_SENSOR_DETECT  \
            (VPS_VID_DEC_IOCTL_BASE + 0x05)
...
typedef struct
{
  ...
  UInt32 serdesEQ;
  /**< ph3100k equalizer level */

  UInt32 serdesPE;
  /**< ph3100k preemphasis level */
...
typedef struct
{
  ...
  UInt8 reg9272;	//# max 9272 register
