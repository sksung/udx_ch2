...
static int eq_lv[MAX_DEV_NUM];		//# serdes equalizer level
static int pe_lv[MAX_DEV_NUM];		//# serdes preemphasis level
...
static Int32 max9271_init(int idx)
{
	...
	eq = eq_lv[idx];
	pe = pe_lv[idx];
	Vps_printf(" [ph3100k] %s serdes_eq:0x%X, serdes_pe:0x%X ###################\n", __func__, eq, pe);
							  ...
							  Vps_VideoDecoderRegRdWrParams *pPrm)
{
	...
	max927x_select(pPrm->deviceNum);
	if(pPrm->reg9272)
		ret = dev_i2c_read8(PH3100K_I2C_ID, MAX9272_I2C_ADDR+pPrm->deviceNum, pPrm->regAddr, &pPrm->regValue);
	else
