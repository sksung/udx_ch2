                                    ...
                                    UInt32 instId)
{
    ...
    if (((VPS_M2M_INST_AUX_SC2_WB1 == instId) ||
         ...
         (FVID2_SOK == retVal))
    {
        if (TRUE == createPrms->enableWriteCtx)
        {
            GT_0trace(VpsMdrvDeiTrace, GT_ERR,
                "Context cannot be rotated for this platform\n");
            retVal = FVID2_EINVALID_PARAMS;
        }
    ...
    if (FVID2_SOK == retVal)
    {
        ...
        for (chCnt = 0u; chCnt < createPrms->numCh; chCnt++)
        {
            if ((FVID2_DF_YUV420SP_UV == inChPrms->inFmt.dataFormat) &&
                (FALSE == createPrms->enableWriteCtx))
            {
                GT_0trace(VpsMdrvDeiTrace, GT_ERR,
                    "This feature is not supported for YUV420 input format\n");
                retVal = FVID2_EINVALID_PARAMS;
                break;
            }
            inChPrms++;
