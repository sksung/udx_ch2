                          ...
                          Int32 *retValPtr)
{
    ...
    else
    {
        ...
        hObj->platformId = Vps_platformGetId();
        if ((hObj->platformId <= VPS_PLATFORM_ID_UNKNOWN) ||
            (hObj->platformId >= VPS_PLATFORM_ID_MAX))
        {
            GT_assert( GT_DEFAULT_MASK, FALSE);
        }
        hObj->cpuRev = Vps_platformGetCpuRev();
        if (hObj->cpuRev >= VPS_PLATFORM_CPU_REV_UNKNOWN)
        {
            GT_assert( GT_DEFAULT_MASK,  FALSE);
        }
    }
    ...
    if (FVID2_SOK == retVal)
    {
        ...
        else
        {
            ...
            {
                hObj->enableWriteCtx = createPrms->enableWriteCtx;
            }
            else
            {
                hObj->enableWriteCtx = FALSE;
            }
...
static Int32 vpsMdrvDeiAllocChObjs(VpsMdrv_DeiHandleObj *hObj, UInt32 numCh)
{
    ...
    for (chCnt = 0u; chCnt < numCh; chCnt++)
    {
        ...
        for (ctxIndex = 0u;
             ctxIndex <= VPSMDRV_DEIH_MAX_FIELD_CONTEXT;
             ctxIndex++)
        {
