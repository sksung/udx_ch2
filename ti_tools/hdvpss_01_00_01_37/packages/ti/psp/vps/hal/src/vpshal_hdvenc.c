...
/* This is a delay of the DAC + Delay introduced by analog part of venc
 * Syncs signals go through only digital part and Data signals go through
 * analog venc for analog outupt. There is skew between sync and data. So to
 * correct that skew artificial delay is introduced in sync using the below
 * two macros.
 */
#if defined(TI_816X_BUILD)
#define ANALOG_PIXEL_CLOCK_DELAY                    (11u)
#define ANALOG_HALF_PIXEL_CLOCK_DELAY               (1u)
#elif defined(TI_8107_BUILD)
#define ANALOG_PIXEL_CLOCK_DELAY                    (12u)
#define ANALOG_HALF_PIXEL_CLOCK_DELAY               (0u)
#else
#define ANALOG_PIXEL_CLOCK_DELAY                    (0u)
#define ANALOG_HALF_PIXEL_CLOCK_DELAY               (4u)
#endif
/*one clock cycle delay is added when color space converter is used*/
#define CSC_DELAY                                   (1u)
/*
...
HdVenc_StdModeInfo HdVencStdModeInfo[] = {
    ...
    {{FVID2_STD_WVGA_60, 480, 800, FVID2_SF_PROGRESSIVE, 30000, 60,
        20, 20, 4, 16, 15, 10}, 1, 6, 0},
    {{FVID2_STD_QVGA_60, 240, 320, FVID2_SF_PROGRESSIVE, 6000, 60,
        8, 43, 41, 3, 12, 10}, 1, 6, 0},
...
Int VpsHal_hdVencSetConfig(VpsHal_Handle handle, VpsHal_HdVencConfig *vencCfg)
{
    ...
    if (VPS_SOK == retVal)
    {
        ...
        regsOvly->CFG53 = ((dCfg->hsShift <<
                                CSL_VPS_HD_VENC_A_CFG53_DVO_HS_SHIFT_SHIFT) &
                                CSL_VPS_HD_VENC_A_CFG53_DVO_HS_SHIFT_MASK) |
                           ((dCfg->vsShift <<
                                CSL_VPS_HD_VENC_A_CFG53_DVO_VS_SHIFT_SHIFT) &
                                CSL_VPS_HD_VENC_A_CFG53_DVO_VS_SHIFT_MASK) |
                           ((dCfg->fidShift <<
                               CSL_VPS_HD_VENC_A_CFG53_DVO_FID_SHIFT_SHIFT) &
                                CSL_VPS_HD_VENC_A_CFG53_DVO_FID_SHIFT_MASK);
                                ...
                                FVID2_ModeInfo *tInfo)
{
            ...
            hdVencObj->outputCfg.fmt))
    {
        ...
        if (VPSHAL_HDVENC_HDCOMP == hdVencObj->instId)
        {
            ...
            if (hdVencObj->outputCfg.dataSource ==
                VPSHAL_HDVENC_OUTPUTSRC_COLORSPC)
            {
               aCfg->hbiPixelEnd = tInfo->hSyncLen + tInfo->hBackPorch -
                                ANALOG_PIXEL_CLOCK_DELAY;
                oCfg->lastNonActivePixelInLine = tInfo->hSyncLen +
                                                tInfo->hBackPorch -
                                                ANALOG_PIXEL_CLOCK_DELAY - 4u;
            }
            else
            {
                   aCfg->hbiPixelEnd = tInfo->hSyncLen + tInfo->hBackPorch -
                                ANALOG_PIXEL_CLOCK_DELAY - CSC_DELAY;
                oCfg->lastNonActivePixelInLine = tInfo->hSyncLen +
                                                tInfo->hBackPorch -
                                                ANALOG_PIXEL_CLOCK_DELAY
                                                - CSC_DELAY - 4u;
            }
            dCfg->hsShift = ANALOG_HALF_PIXEL_CLOCK_DELAY;
            dCfg->vsShift = ANALOG_HALF_PIXEL_CLOCK_DELAY;
            dCfg->fidShift = ANALOG_HALF_PIXEL_CLOCK_DELAY;
