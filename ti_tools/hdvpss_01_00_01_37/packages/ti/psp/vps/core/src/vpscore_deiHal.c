                         ...
                         const Vcore_DescMem *descMem)
{
        ...
        (FVID2_DF_YUV422SP_UV == chObj->corePrms.fmt.dataFormat))
    {
        ...
        {
            /* For TI816x PG2.0, use interlace bypass mode in case of
             * progressive input as progressive mode has 1/3rd performance drop.
             * Hence set the line mode same as that for interlace input. */
            if ((VPS_PLATFORM_ID_EVM_TI816x == platformId) &&
                (cpuRev >= VPS_PLATFORM_CPU_REV_2_0) &&
                (FALSE == chObj->ctxInfo.isTnrMode))
            {
                lineMode[VCORE_DEI_Y_IDX] = VPSHAL_VPDMA_LM_1;
                lineMode[VCORE_DEI_CBCR_IDX] = VPSHAL_VPDMA_LM_1;
            }
                                    ...
                                    Vcore_DeiChObj *chObj)
{
    ...
    if (TRUE == chObj->ctxInfo.isTnrMode)
    {
