...
Int32 Vps_captPrintAdvancedStatistics(UInt32 totalTimeInMsecs)
{
    ...
    Vps_printf("\n"
            ...
            " VIP Parser Reset Count : %d\n"
            "     |  Total        Even         Odd          Min/ Max   Min/Max Dropped\n"
            "  CH |  Fields(FPS)  Fields(FPS)  Fields(FPS)     Width    Height  Fields\n"
            " -------------------------------------------------------------------------\n",
            gVps_captCommonObj.resetCount
	);
    ...
    for(drvId=0; drvId<VPS_CAPT_INST_MAX; drvId++)
    {
        ...
        for(streamId=0; streamId<pDrvObj->createArgs.numStream; streamId++)
        {
            for(chId=0; chId<pDrvObj->createArgs.numCh; chId++)
            {
                ...
                if(totalFieldCount)
                {
                    Vps_printf(" %d%d%d | %7d(%3d) %7d(%3d) %7d(%3d) %4d/%4d %4d/%4d %7d\n",
                        drvId,
                        streamId,
                        chId,
                        totalFieldCount, totalFieldCount/(totalTimeInMsecs/1000),
                        pChObj->fieldCount[0], pChObj->fieldCount[0]/(totalTimeInMsecs/1000),
                        pChObj->fieldCount[1], pChObj->fieldCount[1]/(totalTimeInMsecs/1000),
                        pChObj->minRecvFrameWidth,
                        pChObj->maxRecvFrameWidth,
                        pChObj->minRecvFrameHeight,
                        pChObj->maxRecvFrameHeight,
