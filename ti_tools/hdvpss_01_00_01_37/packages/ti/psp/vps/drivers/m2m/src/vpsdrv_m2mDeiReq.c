...
static Int32 vpsMdrvDeiRotateCtxBuf(VpsMdrv_DeiHandleObj *hObj,
                                    VpsMdrv_DeiChObj *chObj,
                                    UInt32 inFid);
...
Int32 VpsMdrv_deiMlmCbFxn(Mlm_SubmitReqInfo *reqInfo)
{
    ...
    if (VPSMDRV_QOBJ_TYPE_FRAMES == qObj->qObjType)
    {
        ...
        if ((VPS_PLATFORM_ID_EVM_TI814x == hObj->platformId) ||
            (VPS_PLATFORM_ID_EVM_TI8107 == hObj->platformId) ||
            ((VPS_PLATFORM_ID_EVM_TI816x == hObj->platformId) &&
             (FALSE == hObj->enableWriteCtx)))
        {
                                    ...
                                    UInt32 descSet)
{
...
#ifdef TI_816X_BUILD
        else if ((FALSE == hObj->enableWriteCtx) &&
                 (VPS_PLATFORM_ID_EVM_TI816x == hObj->platformId))
        {
            if ((VPS_M2M_INST_MAIN_DEIH_SC1_WB0 == hObj->instObj->drvInstId) ||
                (VPS_M2M_INST_MAIN_DEIH_SC3_VIP0 == hObj->instObj->drvInstId) ||
                (VPS_M2M_INST_MAIN_DEIH_SC1_SC3_WB0_VIP0 == hObj->instObj->drvInstId))
            {
                if (chObj->deiHqCfg.fldMode == VPS_DEIHQ_FLDMODE_4FLD)
                {
                    invertFid = TRUE;
                }
            }
    ...
    for (coreCnt = 0u; coreCnt < VPSMDRV_DEI_MAX_CORE; coreCnt++)
    {
        ...
        if (TRUE == instObj->isCoreReq[coreCnt])
        {
            ...
            if ((coreCnt < VPSMDRV_DEI_MAX_INPUTS) && (invertFid))
            {
                fid = fid ^ 0x1u;
            }
                                    ...
                                    UInt32 inFid)
{
    ...
    if ((VPS_PLATFORM_ID_EVM_TI816x == hObj->platformId) &&
        (TRUE == hObj->enableWriteCtx))
    {
