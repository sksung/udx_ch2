 ...
 */
Int32 vpsMdrvDeiAgeCtxBufs(VpsMdrv_DeiChObj *chObj, FVID2_Frame *curFrame, UInt32 invertFid)
{
    Int32                   retVal = FVID2_SOK;
    UInt32                  currFid, agedTth, numFld;
    ...
    while (TRUE)
    {
         ...
         */
        numFld = chObj->deiCoreCtxInfo.numPrevFld;
        if (TRUE == chObj->isCtxFieldInit)
        {
            /* Age fields */
            agedField =
                chObj->ctxFieldFifo[numFld].fieldCtxBuf;
            agedTth   = chObj->ctxFieldFifo[numFld].tth;

            for (ctxFldIdx = numFld; ctxFldIdx > 0u; ctxFldIdx--)
            {
        ...
        chObj->ctxFieldFifo[0u].tth = numFld;
                                        ...
                                        Ptr cmdArgs)
{
    ...
    if (FVID2_SOK == retVal)
    {
        if (TRUE == hObj->enableWriteCtx)
        {
            GT_0trace(
                VpsMdrvDeiTrace,
                GT_ERR,
                "Ioctl is not supported for this handle\n");
            retVal = FVID2_EUNSUPPORTED_CMD;
        }
    }
    ...
    if (FVID2_SOK == retVal)
    {
        ...
        for (frmCnt = 0u; frmCnt < numFrames; frmCnt++)
        {
             ...
             * copying */
            for (fldCnt = 0u; fldCnt < chObj->deiCoreCtxInfo.numPrevFld; fldCnt++)
            {
            ...
            chObj->ctxFieldFifo[chObj->deiCoreCtxInfo.numPrevFld].fieldCtxBuf =
            ...
            {
                for (fldCnt = 0u;
                     fldCnt <= chObj->deiCoreCtxInfo.numPrevFld;
                     fldCnt++)
                {
                    chObj->ctxFieldFifo[fldCnt].tth =
                        chObj->deiCoreCtxInfo.numPrevFld - fldCnt;
                }
                chObj->isCtxFieldInit = TRUE;
            }
        }
    ...
    while (TRUE)
    {
        ...
        for(chIndex = 0u; chIndex < hObj->numCh; chIndex++)
        {
            ...
            /* For each channel - get the context buffers - including the
               current field */
            for (fifoIndex = 0u;
                 fifoIndex <= chObj->deiCoreCtxInfo.numPrevFld;
                 fifoIndex++)
            {
                ...
                if (chObj->ctxFieldFifo[fifoIndex].tth <=
                        chObj->deiCoreCtxInfo.numPrevFld)
                {
