...
XDAS_Void printProfileNums(XDAS_Int8  chnlNum)
{
  ...
  m3PreCycles  = profilePrms.procStart_hdvicpAcquire[0] +
                 profilePrms.hdvicpAcquire_hdvicpWait[0] ;
  m3PostCycles = profilePrms.hdvicpDone_hdvicpRelease[0] +
                 profilePrms.hdvicpRelease_procEnd[0]  ;

  m3Cycles     = m3PreCycles + m3PostCycles ;
  iVACycles    = profilePrms.hdvicpAcquire_hdvicpWait[0] +
                 profilePrms.hdvicpWait_hdvicpDone[0] +
                 profilePrms.hdvicpDone_hdvicpRelease[0] ;
  
  if(doneCount == 2)
  {
    m3Cycles += profilePrms.hdvicpDone_hdvicpWait2[0];
    iVACycles += profilePrms.hdvicpWait2_hdvicpDone2[0];
  }                  
  
  totalCycles = profilePrms.processStartTime[0];
  /*if((XDAS_Int32)(totalCycles - profilePrms.processEndTime[0])< 0)
  {
    totalCycles += SYSTICKRELVAL;
  } */   
  totalCycles = profilePrms.processEndTime[0] - totalCycles;
  if((profilePrms.processEndTime[0] - profilePrms.processStartTime[0]) < 0)
  {
    totalCycles = profilePrms.processStartTime[0] - profilePrms.processEndTime[0];
  }
  ...
  printf("frameCount: %d, M3 Usage: %3.2f MHz(M3), IVA Usage : %3.2f MHz(IVA), Total usage: %3.2f MHz(IVA)\n",
             ...
             (iVACycles*30.0/1000000), 
             (totalCycles*30.0/1000000));                 
             
  printf(" %d, %d \n", profilePrms.processStartTime[0], profilePrms.processEndTime[0]);
