...
"\t\thostPreIva\tpreMbLoop\tinMbLoop\tpostMbLoop\thostPostIva\tivaTotalCycles\n");
    ...
    do
    {
      ...
      for(i=0;i<numChannels;i++)
      {
        ...
        if(nChannelState[i].validBytes)
        {
          ...
          /*------------------------------------------------------------------*/
          printf("CH:%d # Input ID: %3d, Output ID: %3d, FreeBufID: %3d, PictureCount: %3d \t%s \t%s",i, 
                nChannelState[i].inArgs.viddec3InArgs.inputID, nChannelState[i].outArgs.viddec3OutArgs.outputID[0],
                nChannelState[i].outArgs.viddec3OutArgs.freeBufID[0],
    ...
    for(i=0;i<numChannels;i++)
    {
      ...
      if(nChannelState[i].appCntrlParams.dualTilerEnable)
      {
