                            ...
                            )
{
  ...
  for(Idx=0; Idx<IVIDEO_MAX_NUM_METADATA_PLANES; Idx++)
  {
    ...
    {
      metaBufIdx = 2 + metaDataIndex[chnlNum][Idx] + 
            (2 * nChannelState[chnlNum].params.enableDualOutput);
    ...
    {
      outputBufDesc->descs[Idx + 2 + (2 * 
            nChannelState[chnlNum].params.enableDualOutput)].bufSize.bytes = 0;
      outputBufDesc->descs[Idx + 2 + (2 * 
            nChannelState[chnlNum].params.enableDualOutput)].buf = NULL;
      outputBufDesc->descs[Idx + 2 + (2 * 
            nChannelState[chnlNum].params.enableDualOutput)].memType = 
                                                        XDM_MEMTYPE_ROW;
    }
  }
}
                              ...
                              XDAS_Int8  chnlNum)
{
  ...
  do
  {
    ...
    printf("\n FreeBufID: %3d, FreeBufID: %3d, \n", 
        nChannelState[chnlNum].outArgs.viddec3OutArgs.freeBufID[0],
        nChannelState[chnlNum].outArgs.viddec3OutArgs.freeBufID[1]);
