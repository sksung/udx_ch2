                                ...
                                XDAS_Int32 siNumFramesEncoded)
{
  ...
  printf(":M3 Usage: %3.2f MHz(M3), IVA Usage : %3.2f MHz(IVA)",
             ((m3Cycles*30.0)/1000000), (iVACycles*30*IVAHD_MHZ/M3_HZ));

  fprintf(fTrace_file, ":M3 Usage: %3.2f MHz(M3), IVA Usage : %3.2f MHz(IVA)",
             ((m3Cycles*30.0)/1000000), (iVACycles*30*IVAHD_MHZ/M3_HZ));
...
#ifdef GENERATE_PROFILE_NUMBER
{
  ...
  if(siContentType == 1)
  {
    ...
    printf(" : MB Loop Field 0: %3d MHz", (((uiField0Cycles)*30)/1000000));
    fprintf(fTrace_file, " : MB Loop Field 0: %3d MHz",
                              (((uiField0Cycles)*30)/1000000));
    printf(" : MB Loop Field 1: %3d MHz (%3d Cycles)",
                               (((uiField1Cycles)*30)/1000000), uiField1Cycles);
    fprintf(fTrace_file, " : MB Loop Field 1: %3d MHz",
                              (((uiField1Cycles)*30)/1000000));
  }
  else
  {
