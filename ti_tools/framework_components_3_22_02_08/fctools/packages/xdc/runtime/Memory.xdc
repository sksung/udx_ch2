 ...
 *
 *  All memory allocations are executed via `staticPlace()` at the
 *  config time or through `Memory_alloc()` (or `Memory_calloc()` or
 *  `Memory_valloc()`) at run-time. 
 *
 *  The actual memory management is performed by modules that
 *  implement `{@link xdc.runtime.IHeap}`.  Heap instances are created
 *  statically or dynamically via the heap specific create functions and 
 *  these instances are then passed as input parameters to the `Memory` 
 *  calls with `{@link xdc.runtime.IHeap#Handle}` parameters.
 */
module Memory {
...
/*
 *  @(#) xdc.runtime; 2, 1, 0,371; 2-10-2012 10:18:55; /db/ztree/library/trees/xdc/xdc-y21x/src/packages/
 */
