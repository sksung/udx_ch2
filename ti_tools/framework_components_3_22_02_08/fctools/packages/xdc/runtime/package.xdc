 ...
 *  `{@link LoggerBuf}` and `{@link LoggerSys}` are simple alternative
 *  implementations of the `{@link ILogger}` event "handler" interface. You
 *  can provide more sophisticated or platform-specific
