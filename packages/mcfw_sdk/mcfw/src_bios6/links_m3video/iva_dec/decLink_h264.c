...
static Int32 DecLink_h264DecoderFlush(DecLink_H264Obj * hObj);
...
#if 0
   /*
     HACK!! HACK!!HACK!! HACK!! - START
     There is an issue identified while dynamic resolution change with the
     N-channel Decoder.  The decoder is not populating the extendedError
     correctly.  I mean decoder not always populate the extendedError in
     the channel where it really occurs, randomly it is getting updated
     in some other channels outArgs context. This is a WA to resolve the
     issue. remove onec decoder fix the issue
   */
    ...
    for (reqObjIdx = 0; reqObjIdx < pReqObjBatch->numReqObjsInBatch; reqObjIdx++)
    {
                                                  ...
                                                  IH264VDEC_ERR_PICSIZECHANGE))
        {
            ...
            if ((outArgs->viddec3OutArgs.freeBufID[0] == 0) ||
                (outArgs->viddec3OutArgs.freeBufID[1] == 0))
            {
    ...
    for (reqObjIdx = 0; reqObjIdx < pReqObjBatch->numReqObjsInBatch; reqObjIdx++)
    {
        ...
        if ((outArgs->viddec3OutArgs.freeBufID[0] != 0) &&
            ...
            (extendedError != 0))
        {
            outArgs->viddec3OutArgs.extendedError = extendedError;
        }
    }
   /*
     HACK!! HACK!!HACK!! HACK!! - END
   */
#endif
    for (reqObjIdx = 0; reqObjIdx < pReqObjBatch->numReqObjsInBatch; reqObjIdx++)
    {
            ...
            if (error != XDM_EOK)
            {
                   ...
                   (!DecLink_h264Decoder_checkErr(extendedError, IH264VDEC_ERR_INVALIDPARAM_IGNORE)))
                {
                    ...
                    if (DecLink_h264Decoder_checkErr(outArgs->viddec3OutArgs.
                                                     ...
                                                     IH264VDEC_ERR_PICSIZECHANGE))
                    {
                    Vps_printf ("DEC_LINK: decoder changing resolution \n");
                    }
                    else
                    {
                    ...
                    needReset =
                      DecLink_h264DecoderResetCheck(outArgs->viddec3OutArgs.
                                                    extendedError);
                    status = DecLink_h264DecoderFlush(hObj);
                    if (status == XDM_EOK)
                    {
                        inArgs->viddec3InArgs.numBytes = pReqObj->InBuf->fillLength;
                          ...
                          Int linkID, Int channelID, Int scratchGroupID)
{
    ...
    UTILS_MEMLOG_PRINT(" DECLINK_H264", hObj->memUsed,
