...
EncLink_ChCreateParams ENCLINK_DEFAULTCHCREATEPARAMS_H264 = {
    ...
    .dataLayout = IVIDEO_PROGRESSIVE, //IVIDEO_FIELD_SEPARATED,
...
static EncDec_ResolutionClass enclink_get_resolution_class(UInt32 width, UInt32 height);
...
static Int enclink_add_chinfo_to_resclass(EncDec_ResolutionClass resClass,
									UInt32 chID, encLinkResClassChannelInfo * resClassChInfo);
static Int enclink_compare_resclass_resolution(const void *resInfoA, const void *resInfoB);
...
static Int enclink_populate_outbuf_pool_size_info(EncLink_CreateParams * createArgs,
									System_LinkQueInfo * inQueInfo, EncLink_OutObj * outQueInfo);
...
static Int32 EncLink_codecDynamicResolutionChange(EncLink_Obj * pObj,
									EncLink_ReqObj * pReqObj, UInt32 chId);
static Int32 EncLink_PrepareBatch (EncLink_Obj *pObj, UInt32 tskId,
									EncLink_ReqObj *pReqObj, EncLink_ReqBatch *pReqObjBatch);
...
EncDec_ResolutionClass enclink_get_resolution_class(UInt32 width, UInt32 height)
{
    ...
    UTILS_assert((width <= UTILS_ENCDEC_RESOLUTION_CLASS_10MP_WIDTH)
                 && (height <= UTILS_ENCDEC_RESOLUTION_CLASS_10MP_HEIGHT));
    if ((width > UTILS_ENCDEC_RESOLUTION_CLASS_8MP_WIDTH) ||
        (height > UTILS_ENCDEC_RESOLUTION_CLASS_8MP_HEIGHT))
    {
        resClass = UTILS_ENCDEC_RESOLUTION_CLASS_10MP;
    }
 	else if ((width > UTILS_ENCDEC_RESOLUTION_CLASS_5MP_WIDTH) ||
			(height > UTILS_ENCDEC_RESOLUTION_CLASS_5MP_HEIGHT))
		{
			resClass = UTILS_ENCDEC_RESOLUTION_CLASS_8MP;
		}
                                      ...
                                      UInt32 * pWidth, UInt32 * pHeight)
{
    ...
    switch (resClass)
    {
        case UTILS_ENCDEC_RESOLUTION_CLASS_10MP:
            *pWidth = UTILS_ENCDEC_RESOLUTION_CLASS_10MP_WIDTH;
            *pHeight = UTILS_ENCDEC_RESOLUTION_CLASS_10MP_HEIGHT;
            break;
		case UTILS_ENCDEC_RESOLUTION_CLASS_8MP:
			*pWidth = UTILS_ENCDEC_RESOLUTION_CLASS_8MP_WIDTH;
			*pHeight = UTILS_ENCDEC_RESOLUTION_CLASS_8MP_HEIGHT;
			break;
                                           ...
                                           EncLink_OutObj * outQueInfo)
{
    ...
    Int mvWidth = 0;
    Int mvHeight = 0;
    UInt32 contentType = IVIDEO_INTERLACED;
    EncDec_ResolutionClass resClass;
    ...
    for (i = 0; i < outQueInfo->numAllocPools; i++)
    {
        ...
        for (j = 0; j < resClassChInfo.resInfo[i].numChInResClass; j++)
        {
            ...
            if(chCreateParams->enableAnalyticinfo == 1)
            {
                ...
                contentType =
                 Utils_encdecMapFVID2XDMContentType(inQueInfo->chInfo[chId]
                   .scanFormat);
                 mvFlag = 1;
        ...
        if(mvFlag)
        {
            if (contentType == IVIDEO_INTERLACED)
                outQueInfo->buf_size[i] += ENC_LINK_GET_INTERLACED_MVBUF_SIZE(mvWidth, mvHeight);
            else
                outQueInfo->buf_size[i] += ENC_LINK_GET_PROGRESSIVE_MVBUF_SIZE(mvWidth, mvHeight);
...
static Int32 EncLink_codecCreateOutObj(EncLink_Obj * pObj)
{
    ...
    for (i = 0; i < pOutObj->numAllocPools; i++)
    {
        ...
        for (bufIdx = 0; bufIdx < pOutObj->outNumBufs[i]; bufIdx++)
        {
			...
			pOutObj->outBufs[bufIdx + totalBufCnt].doNotDisplay = FALSE;
    ...
    for (chId = 0u; chId < pObj->inQueInfo.numCh; chId++)
    {
        for (outId = 0u; outId < ENC_LINK_MAX_OUT_QUE; outId++)
        {
            ...
            pOutChInfo->codingformat = pObj->createArgs.chCreateParams[chId].format;
                                                    ...
                                                    UInt32 chId)
{
	...
	#if 1		//# modify enc size
	pChAlgCreatePrm->maxWidth           = pInChInfo->width;
	pChAlgCreatePrm->maxHeight          = pInChInfo->height;
	#else
	if (pInChInfo->width > ENC_LINK_MAX_FRAME_WIDTH)
		pChAlgCreatePrm->maxWidth           = pInChInfo->width;
	else
    pChAlgCreatePrm->maxWidth           = ENC_LINK_MAX_FRAME_WIDTH;
	if (pInChInfo->height > ENC_LINK_MAX_FRAME_HEIGHT)
		pChAlgCreatePrm->maxHeight          = pInChInfo->height;
	else
    pChAlgCreatePrm->maxHeight          = ENC_LINK_MAX_FRAME_HEIGHT;
    #endif
    pChAlgCreatePrm->maxInterFrameInterval = pChDynPrm->interFrameInterval;

	if(pObj->createArgs.vsEnable)
	{
		pChAlgCreatePrm->maxWidth  = (pChAlgCreatePrm->maxWidth * VSTAB_SCALE_DENOMINATOR)/VSTAB_SCALE_NUMERATOR;
		pChAlgCreatePrm->maxHeight = (pChAlgCreatePrm->maxHeight * VSTAB_SCALE_DENOMINATOR)/VSTAB_SCALE_NUMERATOR;
	}
	...
	pChAlgCreatePrm->StreamPreset       = pChCreatePrm->StreamPreset;

   	/* We don't support HIGH SPEED preset for resolution higher than 2K*2K */
   	if ((pInChInfo->width > 2048) || (pInChInfo->height > 2048))
	   	pChAlgCreatePrm->StreamPreset = ALG_VID_ENC_PRESET_HIGHQUALITY;
                                                             ...
                                                             UInt32 chId)
{
    ...
    pChAlgDynPrm->startX        = pInChInfo->startX;
    pChAlgDynPrm->startY        = pInChInfo->startY;
    ...
    {
        pChAlgDynPrm->targetFrameRate = pChDynPrm->outputFrameRate;//ENC_LINK_MJPEG_DEFAULT_ALGPARAMS_TARGETFRAMERATEX5000;
        pChAlgDynPrm->refFrameRate    = pChDynPrm->outputFrameRate;//ENC_LINK_MJPEG_DEFAULT_ALGPARAMS_REFFRAMERATEX5000;
    }
    else
    if (
    ...
    {
        pChAlgDynPrm->targetFrameRate	= pChDynPrm->outputFrameRate;//ENC_LINK_DEFAULT_ALGPARAMS_TARGETFRAMERATEX1000;
        pChAlgDynPrm->refFrameRate		= pChDynPrm->outputFrameRate;//ENC_LINK_DEFAULT_ALGPARAMS_REFFRAMERATEX1000;
    }
    else
    {
...
static Void EncLink_codecProcessTskFxn(UArg arg1, UArg arg2)
{
	...
	while (pObj->state != SYSTEM_LINK_STATE_STOP)
	{
		...
		if (!UTILS_ISERROR(status))
		{
			...
			1. It should have only one type of codec. i.e. All H.264 or all MJPEG
			...
			*/
			...
			status = EncLink_PrepareBatch (pObj, tskId, pReqObj, &pObj->reqObjBatch[tskId]);
			if (UTILS_ISERROR(status)) {
				UTILS_warn("ENCLINK:ERROR in EncLink_PrepareBatch.Status[%d]", status);
			}
		}
		...
		if (pObj->reqObjBatch[tskId].numReqObjsInBatch)
		{
			...
			if (UTILS_ISERROR(status)) {
				UTILS_warn("ENC : IVAHDID : %d ENCLINK:ERROR in EncLink_SubmitBatch.Status[%d]", tskId, status);
			}
			else
			{
				...
				pObj->batchStatistics[tskId].currentBatchSize =
					pObj->reqObjBatch[tskId].numReqObjsInBatch;
		...
		{
			/* Return the processed ReqObjects to the Enc Link via the ProcessDone Queue*/
...
#pragma DATA_SECTION(gEncProcessTskStack, ".bss:taskStackSection")
...
Int32 EncLink_codecCreate(EncLink_Obj * pObj, EncLink_CreateParams * pPrm)
{
    ...
    for (chId = 0; chId < pObj->inQueInfo.numCh; chId++)
    {
        ...
        #ifdef SYSTEM_DEBUG_ENC
        Vps_printf(" ENCODE: CH%d | %d x %d, [%s] bitrate = %d Kbps, (%d/%d) fps\n",
                chId,
                ...
                pObj->createArgs.chCreateParams[chId].defaultDynamicParams.inputFrameRate,
                pObj->createArgs.chCreateParams[chId].defaultDynamicParams.outputFrameRate/1000
		);
        #endif
    ...
    /* Motion Detection Alg create */
    status = DM81XX_MD_Create();
	...
	//UTILS_MEMLOG_PRINT(" ENCLINK", pObj->memUsed, UTILS_ARRAYSIZE(pObj->memUsed));
...
Int32 EncLink_codecDelete(EncLink_Obj * pObj)
{
    ...
    ENCLINK_INFO_LOG(pObj->linkId, -1, " ENCODE: Delete in progress !!!\n");
    ...
    for (chId = 0; chId < pObj->inQueInfo.numCh; chId++)
    {
        ...
        #if 0//def SYSTEM_DEBUG_ENC
        Vps_printf(" %d: ENCODE: CH%d: totalIn Frames : %d, Dropped Frames: %d, FPS: %d\n",
                    Utils_getCurTimeInMsec(), chId,
                    pChObj->totalInFrameCnt,
                    pChObj->inFrameSkipCount,
                    (pChObj->curFrameNum - pChObj->inFrameSkipCount) / (pChObj->totalFrameIntervalTime/1000)
                 );
        #endif
    ...
    /* Motion Detection Alg delete */
    DM81XX_MD_Delete();

    ENCLINK_INFO_LOG(pObj->linkId, -1, " ENCODE: Delete done !!!\n");
...
Int32 EncLink_printStatistics (EncLink_Obj * pObj, Bool resetAfterPrint)
{
	...
	{
        pChObj = &pObj->chObj[chId];
		Vps_printf(" [i] ENC: CH(%d) | Recv FPS(%d), Skip FPS(%d), Out FPS(%d)\n",
                   chId,
	...
	{
        pChObj = &pObj->chObj[chId];
		Vps_printf(" [i] ENC: CH(%d) | Recv(%d), rejet(%d), skip(%d), Out(%d, %dfps)\n",
                   chId,
                   pChObj->inFrameRecvCount,
                   pChObj->inFrameRejectCount,
                   pChObj->inFrameUserSkipCount,
                   pChObj->outFrameCount, pChObj->outFrameCount / elaspedTime);
	}
	#endif
                                  ...
                                  EncLink_ChRateControlParams * params)
{
    ...
    key = Hwi_disable();
    pChObj = &pObj->chObj[params->chId];
    pChObj->algObj.algDynamicParams.rateControl = params->rateControl;
                                 ...
                                 EncLink_ChPacketSizeParams * params)
{
    ...
    key = Hwi_disable();
    pChObj = &pObj->chObj[params->chId];
    pChObj->algObj.algDynamicParams.packetSize = params->packetSize;
    pChObj->algObj.setConfigBitMask |=
        (1 << ENC_LINK_SETCONFIG_BITMASK_PACKETSIZE);
    Hwi_restore(key);
                                   ...
                                   EncLink_ChEncodePresetParams * params)
{
    ...
    key = Hwi_disable();
    pChObj = &pObj->chObj[params->chId];
    pChObj->algObj.algDynamicParams.encodePreset = params->encodePreset;
	/* We don't support HIGH SPEED preset for resolution higher than 2K*2K */
	if ((pChObj->algObj.algDynamicParams.inputWidth > 2000) ||
		(pChObj->algObj.algDynamicParams.inputHeight > 2000))
		pChObj->algObj.algDynamicParams.encodePreset = ALG_VID_ENC_PRESET_HIGHQUALITY;
    pChObj->algObj.setConfigBitMask |=
...
Int32 EncLink_codecSetROIPrms(EncLink_Obj * pObj, EncLink_ChROIParams * params)
{
    ...
    for (i = 0; i < 3; i++)
    {
        pChObj->algObj.algDynamicParams.roiNumOfRegion = params->numOfRegion;
        pChObj->algObj.algDynamicParams.roiStartX[i] = params->startX[i];
        pChObj->algObj.algDynamicParams.roiStartY[i] = params->startY[i];
        pChObj->algObj.algDynamicParams.roiWidth[i] = params->width[i];
        pChObj->algObj.algDynamicParams.roiHeight[i] = params->height[i];
        pChObj->algObj.algDynamicParams.roiType[i] = params->type[i];
    }
...
Int32 EncLink_codecSetInputKeyPrms(EncLink_Obj * pObj, EncLink_ChInputKeyParams * params)
{
    ...
    pChObj->algObj.algDynamicParams.inputKey = params->inputKey;

    pChObj->algObj.setConfigBitMask |= (1 << ENC_LINK_SETCONFIG_BITMASK_INPUTKEY);
    Hwi_restore(key);
...
Bool  EncLink_doSkipFrame(EncLink_ChObj *pChObj, Int32 chId)
{
    ...
    if(pChObj->frameStatus.firstTime)
    {
        ...
        #if 0
        Vps_printf("\r\n CH%02d: inputframerate:%d targetfps:%d", chId,
			pChObj->inputFrameRate, (pChObj->algObj.algDynamicParams.targetFrameRate/1000));
		#endif
    }
                                   ...
                                   EncLink_ReqBatch *pReqObjBatch)
{
	...
	if ((UTILS_ENCDEC_RESOLUTION_CLASS_10MP_WIDTH >= inputFrameWidth) &&
	...
	switch (pChObj->algObj.algCreateParams.format)
	{
		...
		case IVIDEO_H264HP:
			pReqObjBatch->codecSubmittedFlag = pReqObjBatch->codecSubmittedFlag |
                                              (0x1 << ENC_LINK_GROUP_CODEC_CLASS_H264);
			break;

		case IVIDEO_MJPEG:
			pReqObjBatch->codecSubmittedFlag = pReqObjBatch->codecSubmittedFlag |
                                              (0x1 << ENC_LINK_GROUP_CODEC_CLASS_JPEG);
			break;
	...
	while (FALSE == batchPreperationDone)
	{
		...
		if (Utils_queIsEmpty (&pObj->encProcessTsk[tskId].processQue))
		{
			...
			#ifdef SYSTEM_DEBUG_MULTI_CHANNEL_ENC
			Vps_printf ("ENC : IVAHDID : %d Incoming Queue is empty. Batch generation complete!!", tskId);
			#endif
			batchPreperationDone = TRUE;
			continue;
		}
		...
		status = Utils_quePeek(&pObj->encProcessTsk[tskId].processQue, (Ptr *) &pNewReqObj);
		...
		if (contentType != Utils_encdecMapFVID2XDMContentType(
									pObj->inQueInfo.chInfo[newObjChannelId].scanFormat))
		{
		...
		if (! codecClassSwitch)
		{
			...
			#ifdef SYSTEM_DEBUG_MULTI_CHANNEL_ENC
			Vps_printf ("ENC : IVAHDID : %d Codec Switch occured!!. Batch generation complete!!", tskId);
			#endif
			batchPreperationDone = TRUE;
			continue;
		}
	...
	if (FVID2_SOK == status)
	{
		...
		{
			Vps_printf ("ENC : IVAHDID : %d %d\n", tskId, pReqObjBatch->pReqObj[i]->OutBuf->channelNum);
		}
		#endif
	}
...
static Int32 EncLink_SubmitBatch (EncLink_Obj *pObj, UInt32 tskId, EncLink_ReqBatch *pReqObjBatch)
{
	...
	for (i = 0; i < pReqObjBatch->numReqObjsInBatch; i++)
	{
		...
		switch (format)
		{
			...
			case IVIDEO_H264HP:
				...
				if (UTILS_ISERROR(status)) {
					UTILS_warn("ENCLINK:ERROR in Enclink_h264EncodeFrame.Status[%d]", status);
				}
				break;
			...
			case IVIDEO_MJPEG:
				...
				if (UTILS_ISERROR(status)) {
					UTILS_warn("ENCLINK:ERROR in Enclink_JPEGEncodeFrame.Status[%d]", status);
				}
				break;
	...
	switch (format)
	{
        ...
        case IVIDEO_H264HP:
            ...
            if (UTILS_ISERROR(status)) {
                UTILS_warn("ENCLINK:ERROR in "
                           "Enclink_h264EncodeFrameBatch.Status[%d] for IVAHD_%d", status, tskId);
            }
            break;
