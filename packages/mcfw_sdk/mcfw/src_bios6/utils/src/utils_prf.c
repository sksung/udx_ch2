...
Int32 Utils_prfTsPrint(Utils_PrfTsHndl * pHndl, Bool resetAfterPrint)
{
	...
	Vps_printf(" %d: PRF : %s : t: %d ms, c: %d, f: %d, fps: %d, fpc: %d \r\n",
			Utils_getCurTimeInMsec(), pHndl->name, timeMs,	/* in msecs */
			pHndl->count, pHndl->numFrames, fps,			/* frames per second */
...
Int32 Utils_prfLoadPrintAll(Bool printTskLoad)
{
    ...
    Vps_printf(" ### LOAD: CPU: %d%% HWI: %d%%, SWI:%d%% \r\n",  cpuLoad, hwiLoad, swiLoad);
    ...
    if (printTskLoad)
    {
        for (hndlId = 0; hndlId < UTILS_PRF_MAX_HNDL; hndlId++)
        {
            ...
            {
                tskLoad = -1;
                tskLoad = (pHndl->totalTskThreadTime * 100) / gUtils_accPrfLoadObj.totalTime;
                if (tskLoad) {
                    Vps_printf(" ### LOAD: TSK: %s: %d%% \r\n", pHndl->name, tskLoad);
                }
            }
        }
    }
