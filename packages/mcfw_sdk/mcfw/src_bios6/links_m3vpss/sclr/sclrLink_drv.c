...
Int32 SclrLink_drvCreateChObj(SclrLink_Obj * pObj, UInt32 chId)
{
    ...
    pChObj->sclrRtOutFrmPrm.width = pObj->outObj.outFormat.width;
    pChObj->sclrRtOutFrmPrm.height = pObj->outObj.outFormat.height;
    pChObj->sclrRtOutFrmPrm.pitch[0] = pObj->outObj.outFormat.pitch[0];
    pChObj->sclrRtOutFrmPrm.pitch[1] = pObj->outObj.outFormat.pitch[1];
    pChObj->sclrRtOutFrmPrm.pitch[2] = pObj->outObj.outFormat.pitch[2];
...
//    SclrLink_drvCreateScDrv(pObj);
    ...
    UTILS_MEMLOG_PRINT(" SCLR", pObj->memUsed, UTILS_ARRAYSIZE(pObj->memUsed));
