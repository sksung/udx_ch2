...
Int32 System_dispCheckStopList()
{
    ...
    */
    ...
    {
    	#if 1	//# logo stop
    	status = *(volatile UInt32 *)VPDMA_LIST_ATTR;
        *(volatile UInt32 *)VPDMA_LIST_ATTR = status | (VPDMA_LIST_NUMBER_DVO2<<24) | (1<<20);
    	status = *(volatile UInt32 *)VPDMA_LIST_ATTR;
        *(volatile UInt32 *)VPDMA_LIST_ATTR = status | (VPDMA_LIST_NUMBER_HD<<24) | (1<<20);
        //status = *(volatile UInt32 *)VPDMA_LIST_ATTR;
        //*(volatile UInt32 *)VPDMA_LIST_ATTR = status | (VPDMA_LIST_NUMBER_SD<<24) | (1<<20);
        #else
        /* VPSS is powered ON */
        do
        {
