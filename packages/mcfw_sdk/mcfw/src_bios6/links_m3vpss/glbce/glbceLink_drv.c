...
static Int32 GlbceLink_drvInitCh(GlbceLink_Obj * pObj)
{
    ...
    for (chId = 0; chId < pObj->inQueInfo.numCh; chId++)
    {
		...
		pObj->frequencyTonalCurve =  (chCreateParams->preset == GLBCE_HIGHQUALITY) ? 3 : 10;//chCreateParams->frequencyTonalCurve;
		pObj->frameNum = 0;
...
Int32 GlbceLink_drvStop(GlbceLink_Obj * pObj)
{
    ...
    GLBCELINK_INFO_LOG("Stop in progress !!!\n");
