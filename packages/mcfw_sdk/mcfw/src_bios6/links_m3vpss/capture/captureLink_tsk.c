...
UInt8 gCaptureLink_tskStack[2][CAPTURE_LINK_TSK_STACK_SIZE];
    ...
    *pMsg = NULL;
    ...
    while (!runDone)
    {
        ...
        switch (cmd)
        {
			...
			case CAPTURE_LINK_CMD_CHANGE_COLOR:		//# video decoder
				params = (CaptureLink_ColorParams *)Utils_msgGetPrm(pRunMsg);

				/* ACK or free message before proceding */
                Utils_tskAckOrFreeMsg(pRunMsg, status);

                CaptureLink_drvSetColor(pObj, params->contrast, params->brightness, params->saturation, params->chId);
				break;
            ...
            case CAPTURE_LINK_CMD_DETECT_VIDEO:
                ...
                CaptureLink_drvDetectVideo(pObj, FALSE, TRUE, FALSE);
                break;
            ...
            case CAPTURE_LINK_CMD_VIDEO_REG_WRITE:
            {
            	Vps_VideoDecoderRegRdWrParams *regPrm;
            	regPrm = (Vps_VideoDecoderRegRdWrParams *)Utils_msgGetPrm(pRunMsg);

            	status = CaptureLink_drvWriteSensor(pObj, regPrm);
                Utils_tskAckOrFreeMsg(pRunMsg, status);
                break;
			}
			case CAPTURE_LINK_CMD_VIDEO_REG_READ:
			{
				Vps_VideoDecoderRegRdWrParams *regPrm;
				regPrm = (Vps_VideoDecoderRegRdWrParams *)Utils_msgGetPrm(pRunMsg);

            	status = CaptureLink_drvReadSensor(pObj, regPrm);
                Utils_tskAckOrFreeMsg(pRunMsg, status);
                break;
			}
            ...
            case CAPTURE_LINK_CMD_CONFIGURE_BLIND_AREA:
                ...
                CapterLink_drvBlindAreaConfigure(pObj, blindInfo->streamId, blindInfo->channelId, blindInfo);
...
Void CaptureLink_tskMain(struct Utils_TskHndl * pTsk, Utils_MsgHndl * pMsg)
{
    ...
    {
        /* detect video source, remain in READY state */
        status = CaptureLink_drvConfigureVideoDecoder(pObj,(UInt32) BIOS_WAIT_FOREVER);
    }
...
Void CaptureLink_tskDetect(struct Utils_TskHndl *pTsk, Utils_MsgHndl *pMsg)
{
	UInt32 cmd = Utils_msgGetCmd(pMsg);
	CaptureLink_Obj *pObj;
    ...
    pObj->detectStop = FALSE;
	while (!pObj->detectStop)
	{
		CaptureLink_drvDetectSensor(pObj);
		Task_sleep(500);
	}
...
Int32 CaptureLink_init(void)
{
    ...
    */
    ...
    status = Utils_tskCreate(&pObj->tsk,
                             ...
                             gCaptureLink_tskStack[0],
                             CAPTURE_LINK_TSK_STACK_SIZE, pObj, "CAPTURE ");
    UTILS_assert(status == FVID2_SOK);

	//# for hd sensor detect
	status = Utils_tskCreate(&pObj->tsk_det,
                             CaptureLink_tskDetect,
                             4,//CAPTURE_LINK_TSK_PRI-2,
                             gCaptureLink_tskStack[1],
                             CAPTURE_LINK_TSK_STACK_SIZE, pObj, "CAPTURE ");
    UTILS_assert(status == FVID2_SOK);
