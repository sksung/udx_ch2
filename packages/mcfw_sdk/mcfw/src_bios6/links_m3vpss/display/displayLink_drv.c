                                 ...
                                 FVID2_FrameList *freeFrameList)
{
    ...
    if (rtParams)
    {
        UTILS_COMPILETIME_ASSERT(offsetof(DisplayLink_drvRtParams,dispRtPrms) == 0);
        UTILS_assert(UTILS_ARRAYISVALIDENTRY(rtParams,pObj->rtParams.paramsMem));
        status = Utils_quePut(&pObj->rtParams.freeQ,
                              rtParams, BIOS_NO_WAIT);
        UTILS_assert(status == 0);
    }
}
...
Void DisplayLink_changeOriAddr(DisplayLink_Obj *pObj, FVID2_Frame *pFrame)
{
	DisplayLink_FrameInfo *frameInfo;

	if ((pObj->displayCreateArgs.memType == VPS_VPDMA_MT_TILEDMEM) &&
		((FVID2_DF_YUV420SP_UV == pObj->displayFormat.dataFormat) ||
		 (FVID2_DF_YUV420SP_VU == pObj->displayFormat.dataFormat)) &&
		 (TRUE == pObj->createArgs.enable90DegRotate))
	{
		frameInfo = pFrame->appData;

		frameInfo->addr[0][0] = pFrame->addr[0][0];
		frameInfo->addr[0][1] = pFrame->addr[0][1];

		pFrame->addr[0][0] =
			(Ptr)Utils_tilerGetOriAddr(
				(UInt32)pFrame->addr[0][0],
				UTILS_TILER_CNT_8BIT,
				(UTILS_TILER_ORI_XY_SWAP|0x2),	//# 90 rotate
				pObj->displayFormat.width,
				pObj->displayFormat.height);
		pFrame->addr[0][1] =
			(Ptr)Utils_tilerGetOriAddr(
				(UInt32)pFrame->addr[0][1],
				UTILS_TILER_CNT_16BIT,
				(UTILS_TILER_ORI_XY_SWAP|0x2),
				pObj->displayFormat.width,
				pObj->displayFormat.height/2);

		frameInfo->chAddr = 1;
	}
}

Void DisplayLink_revertOriAddr(DisplayLink_Obj *pObj, FVID2_Frame *pFrame)
{
	DisplayLink_FrameInfo *frameInfo;
	...
	if (frameInfo->chAddr)
	{
		pFrame->addr[0][0] = frameInfo->addr[0][0];
		pFrame->addr[0][1] = frameInfo->addr[0][1];
	}
}
...
Vps_DeiDispRtParams *DisplayLink_drvSetDeiFramePitchRtParams(DisplayLink_Obj * pObj)
{
	...
	if ((pObj->displayCreateArgs.memType == VPS_VPDMA_MT_TILEDMEM) &&
		((FVID2_DF_YUV420SP_UV == pObj->displayFormat.dataFormat) ||
	 	...
	 	(TRUE == pObj->createArgs.enable90DegRotate))
	{
		wi = pObj->displayFormat.height;
		he = pObj->displayFormat.width;
	}
	else
	{
		wi = pObj->displayFormat.width;
		he = pObj->displayFormat.height;
	}

	rtParams->deiDispRtPrms.inFrmPrms    = &rtParams->inFrmPrms;

    rtParams->inFrmPrms.width      = wi;
    rtParams->inFrmPrms.height     = he;
	rtParams->inFrmPrms.memType    = pObj->displayCreateArgs.memType;
	rtParams->inFrmPrms.dataFormat = pObj->displayFormat.dataFormat;
    for (i = 0; i < pObj->numDisplayBufPlanes; i++)
    {
                                              ...
                                              FVID2_FrameList *frameList)
{
	...
	DisplayLink_changeOriAddr(pObj, frame);

    if (updateDrvPitch)
    {
        if (DisplayLink_drvIsDeiDisplayDrv(pObj)) {
            frameList->perListCfg = DisplayLink_drvSetDeiFramePitchRtParams(pObj);
        }
        else {
            frameList->perListCfg = DisplayLink_drvSetFramePitchRtParams(pObj);
        }
    }
}
                             ...
                             UInt32 *freeQueId)
{
    ...
    *freeQueId = DISPLAY_LINK_INVALID_INQUEID;
    ...
    status = FVID2_dequeue(pObj->displayHndl,&tmpFreeFrameList,0,
                      BIOS_NO_WAIT);
    if (FVID2_SOK == status)
    {
        ...
        if (tmpFreeFrameList.numFrames)
        {
            ...
            Vps_printf(" %d: DISPLAY: Deque %d frames\n",
                       Utils_getCurTimeInMsec(), tmpFreeFrameList.numFrames);
			#endif
        ...
        for (i = 0; i < tmpFreeFrameList.numFrames;i++)
        {
            ...
            if (DISPLAY_LINK_BLANK_FRAME_CHANNEL_NUM != frame->channelNum)
            {
				DisplayLink_revertOriAddr(pObj, frame);
                ...
                *freeQueId = frameInfo->activeQueueId;
                ...
                else
                {
                    ...
                    UTILS_assert(freeFrameList->numFrames <
                                 UTILS_ARRAYSIZE(freeFrameList->frames));
                    freeFrameList->frames[freeFrameList->numFrames] = frame;
                    freeFrameList->numFrames++;
                    pObj->numBufsInDriver--;
                }
                status = Utils_quePut(&pObj->frameInfo.infoQ,
                               frameInfo, BIOS_NO_WAIT);
                UTILS_assert(status == 0);
            }
        }
    }
    ...
    for (numQue = 0; numQue < DISPLAY_LINK_BLANK_FRAME_INIT_QUE; numQue++)
    {
        ...
        if(pObj->displayCreateArgs.memType != VPS_VPDMA_MT_TILEDMEM)
        {
            pObj->blankFrame.addr[1][0] =
            	(UInt8 *) pObj->blankFrame.addr[0][0] + pObj->displayFormat.pitch[0];
        }
        else
        {
...
Int32 DisplayLink_drvSetDeiDispPrms(DisplayLink_Obj *pObj, FVID2_Format *pFormat)
{
	...
	if ((pObj->displayCreateArgs.memType == VPS_VPDMA_MT_TILEDMEM) &&
		((FVID2_DF_YUV420SP_UV == pObj->displayFormat.dataFormat) ||
	 	...
	 	(TRUE == pObj->createArgs.enable90DegRotate))
	{
		swpSize = deiDispPrms->fmt.width;
		deiDispPrms->fmt.width = deiDispPrms->fmt.height;
		deiDispPrms->fmt.height = swpSize;
	}
    ...
    deiDispPrms->scTarWidth = deiDispPrms->fmt.width;
    deiDispPrms->scTarHeight = deiDispPrms->fmt.height;
...
Int32 DisplayLink_drvDisplayCreate(DisplayLink_Obj * pObj)
{
    ...
    switch (pObj->tskId)
    {
        ...
        case SYSTEM_LINK_ID_DISPLAY_1:
            pObj->displayInstId = VPS_DISP_INST_AUX_SC2;//VPS_DISP_INST_BP1;
            pFormat->scanFormat = FVID2_SF_PROGRESSIVE;
            break;
        case SYSTEM_LINK_ID_DISPLAY_2:
            pObj->displayInstId = VPS_DISP_INST_BP1;//VPS_DISP_INST_SEC1;
...
{
    char *displayName[] = { "BP0", "BP1", "SEC1", "MAIN_DEIH_SC1", "AUX_DEI_SC2", "MAIN_DEI_SC1", "AUX_SC2"};

    Vps_rprintf(" [i] DISPLAY: %s: %d fps, Latency (Min/Max)=(%d/%d)\r\n",
		/*Utils_getCurTimeInMsec(),*/
		displayName[pObj->displayInstId],
		pObj->cbCount*1000/elaspedTime,
		pObj->minLatency, pObj->maxLatency
	);
...
Int32 DisplayLink_drvProcessData(DisplayLink_Obj * pObj)
{
    ...
    if(pObj->isDisplayRunning)
    {
	    ...
	    do
	    {
	        ...
	        status = DisplayLink_drvDeQueue(pObj, &freeFrameList, &freeQueueId);

	        if (freeFrameList.numFrames)
	        {
	            pObj->inFramePutCount += freeFrameList.numFrames;
	            UTILS_assert(freeQueueId < pObj->createArgs.numInputQueues);
	    ...
	    for (i = 0; i < pObj->createArgs.numInputQueues;i++)
	    {
	    	//Vps_printf("--- %s numInputQueues:%d\n", __func__, pObj->createArgs.numInputQueues);
	    ...
	    DisplayLink_drvLock(pObj);
	    frameIdx = 0;
	    while((pObj->maxQueueCount) && (frameIdx < frameList.numFrames))
	    {
			...
			if (pFrame->channelNum == pObj->curDisplayChannelNum)
			{
	            ...
	            UTILS_assert(DISPLAY_LINK_BLANK_FRAME_CHANNEL_NUM != pFrame->channelNum);

	            pFrame->addr[1][0] = (UInt8 *) pFrame->addr[0][0] + pitch0;
	            pFrame->addr[1][1] = (UInt8 *) pFrame->addr[0][1] + pitch1;
	            ...
	            DisplayLink_drvSetFrameInfo(pObj, &pFrame, &frameReject);
	            if (pFrame)
	            {
	                if (frameReject)
	                {
	                ...
	                else
	                {
	                    ...
	                    if(status!=FVID2_SOK)
	                    {
	                        ...
	                        */
	                        freeFrameList.frames[freeFrameNum] = pFrame;
	                        freeFrameNum++;
	                        DisplayLink_drvFreeFrameInfo(pObj, &displayFrameList);
						}
					}
				}
			}
...
#ifdef SYSTEM_DEBUG_DISPLAY
    Vps_printf(" %d: DISPLAY: Start Done (0x%x) !!!\n", Utils_getCurTimeInMsec(), status);
#endif
