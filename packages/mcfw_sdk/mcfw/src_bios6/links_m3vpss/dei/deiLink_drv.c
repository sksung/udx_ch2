...
Int32 DeiLink_drvCreate(DeiLink_Obj * pObj, DeiLink_CreateParams * pPrm)
{
    ...
    UTILS_MEMLOG_PRINT(" DEI", pObj->memUsed, UTILS_ARRAYSIZE(pObj->memUsed));
...

        {
                ...
                (pObj->createArgs.generateBlankOut[outId] == FALSE))
            {
                ...
                else
                {
					...
					#ifdef ENABLE_TILER
					if(pObj->createArgs.tilerEnable) {
						/* UTILS_TILER_ORI_NONE, UTILS_TILER_ORI_X_FLIP, UTILS_TILER_ORI_Y_FLIP */
						pOutFrame->addr[0][0] = (Ptr)Utils_tilerGetOriAddr(
							   (UInt32) pOutFrame->addr[1][0],
							   UTILS_TILER_CNT_8BIT,
							   UTILS_TILER_ORI_NONE,
							   pFormat->width,
							   pFormat->height);
						pOutFrame->addr[0][1] = (Ptr)Utils_tilerGetOriAddr(
							   (UInt32) pOutFrame->addr[1][1],
        ...
        if (pObj->createArgs.enableOut[outputId])
        {
                 ...
                 frameId++)
            {
                ...
                if ((pFrame) && (pFrame != &pObj->outFrameDrop))
                {
					...
					#ifdef ENABLE_TILER
					if(pObj->createArgs.tilerEnable) {
						pFrame->addr[0][0] = pFrame->addr[1][0];
						pFrame->addr[0][1] = pFrame->addr[1][1];
					}
					#endif
                    status = Utils_bufPutFullFrame(&pObj->outObj[outId].bufOutQue, pFrame);
                    UTILS_assert(status == FVID2_SOK);
