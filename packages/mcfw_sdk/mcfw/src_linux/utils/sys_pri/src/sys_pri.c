...
//#define	dprintf(x...) printf(" [sys_pri] "	x)
#define dprintf(x...)
...
{
  dprintf("\n");
  dprintf(" TI816x System Prioirty read/write utility, (c) Texas Instruments 2010\n");
  dprintf(" Built on %s %s\n", __DATE__, __TIME__);
  dprintf(" \n");
  dprintf(" %s --printAll # Prints all system priority setting info\n", str);
  dprintf(" %s --regDump  # Prints all register values related to system priority setting\n", str);
  dprintf(" \n");
  dprintf(" %s --dmm-pri-set <dmm-initiator-name> <dmm-priority> \n"
         " # Set a DMM priority for a specified DMM initiator \n", str);
  dprintf(" \n");
  #ifdef L3_STATIC_PRESSURE_SET_ENABLE
  dprintf(" %s --L3-pri-set <L3-initiator-name> <L3-static-pressure> \n"
         " # Set a L3 static pressure for a specified L3 initiator \n", str);
  dprintf(" \n");
  #endif
  dprintf(" %s --L3-bw-reg-set <L3-bw-reg-initiator-name> <L3-pressure-High> <L3-pressure-Low> <L3-Bandwidth> <L3-Watermark-cycles> \n"
         " # Setup L3 Bandwidth regulator for a specified L3 initiator \n",
                str);
  dprintf(" \n");
  dprintf(" Valid values for \"dmm-initiator-name\" are,\n");
  dprintf(" A8       SYS_MMU  DUCATI   EDMA_RD0 EDMA_RD1 EDMA_RD2 EDMA_RD3\n");
  dprintf(" EDMA_WR0 EDMA_WR1 EDMA_WR2 EDMA_WR3 SGX      HDVICP0  HDVICP1 \n");
  dprintf(" HDVICP2  GMAC0    GMAC1    USB_DMA  USB_QMGR SATA     PCIE    \n");
  dprintf(" \n");
  #ifdef L3_STATIC_PRESSURE_SET_ENABLE
  dprintf(" Valid values for \"L3-initiator-name\" are,\n");
  dprintf(" A8       DSP_CFG  HDVPSS0  HDVPSS1  EDMA_RD2 EDMA_WR2 EDMA_RD3\n");
  dprintf(" EDMA_WR3 GMAC0    GMAC1    USB_DMA  USB_QMGR SATA     DUCATI  \n");
  dprintf(" PCIE \n");
  dprintf(" \n");
  #endif
  dprintf(" Valid values for \"L3-bw-reg-initiator-name\" are,\n");
  dprintf(" HDVICP0  HDVICP1  HDVICP2  EDMA_RD0 EDMA_WR0 EDMA_RD1 EDMA_WR1\n");
  dprintf(" DSP      SGX   \n");
  dprintf(" \n");
  dprintf(" Valid values for \"dmm-priority\"       are, 0..7 (0 is highest priority)\n");
  #ifdef L3_STATIC_PRESSURE_SET_ENABLE
  dprintf(" Valid values for \"L3-static-pressure\" are, 0, 1, 3 (0 is lowest priority)\n");
  #endif
  dprintf(" Valid values for \"L3-pressure-High\"   are, 0, 1, 3 (0 is lowest priority)\n");
  dprintf(" Valid values for \"L3-pressure-Low\"    are, 0, 1, 3 (0 is lowest priority)\n");
  dprintf(" Valid values for \"L3-Bandwidth\" (in MB/s) are, 0..%u\n", 0x1FFF);
  dprintf(" Valid values for \"L3-Watermark\" (in cycles) are, 0..%d\n", 0xFFF);
  dprintf(" \n");
  dprintf(" IMPORTANT NOTE: \n"
...
int SYS_PRI_mapMem()
{
  ...
  if(memDevFd < 0)
  {
    dprintf(" ERROR: /dev/mem open failed !!!\n");
    return -1;
  }
  ...
  for(i=0; i<BW_REG_MAX; i++)
  {
    ...
    if(gSYS_PRI_ctrl.doRegDump)
    {
      dprintf(" BW_REGULATOR[%d]  @ 0x%08x = 0x%08x 0x%08x 0x%08x 0x%08x\n", i, (REG32)gSYS_PRI_ctrl.bwReg[i],
...
int SYS_PRI_dmmPriSet(char *name, int pri)
{
  ...
  {
    dprintf(" ERROR: Unknown DMM initiator [%s] \n", name);
    setReg = 0;
  }
  ...
  if(setReg)
  {
    ...
    dprintf(" Setting DMM priority for [%-8s] to [%d] ( 0x%08x = 0x%08x )\n", name, pri, (unsigned int)pAddr, value);
...
int SYS_PRI_l3StaticPressureSet(char *name, int staticPressure)
{
  ...
  {
    dprintf(" ERROR: Unknown L3 static pressure initiator [%s] \n", name);
    setReg = FALSE;
  }

  if(setReg)
  {
    dprintf(" Setting L3 static pressure for [%-8s] to [%d]\n", name, staticPressure);
...
int SYS_PRI_l3BwRegSet(char *name, int pressH, int pressL, int bandwidth, int watermark)
{
  ...
  if(setReg)
  {
    dprintf(" Setting L3 bandwidth regulator for [%-8s] to [press=[%d,%d] BW=%d, WM Cycles=%d]\n",
...
int SYS_PRI_doPrintAll()
{
  ...
  dprintf(" \n");
  dprintf(" L3 Static Pressure \n");
  dprintf(" ================== \n");
  dprintf(" A8       DSP_CFG  HDVPSS0  HDVPSS1  EDMA_RD2 EDMA_WR2 EDMA_RD3 EDMA_WR3 \n");
  dprintf(" %-8d %-8d %-8d %-8d %-8d %-8d %-8d %-8d\n",
  ...
  dprintf(" \n");
  dprintf(" GMAC0    GMAC1    USB_DMA  USB_QMGR SATA     DUCATI   PCIE\n");
  dprintf(" %-8d %-8d %-8d %-8d %-8d %-8d %-8d\n",
  ...
  dprintf(" \n");
  dprintf(" DMM Priority \n");
  dprintf(" ============ \n");
  dprintf(" A8       SYS_MMU  DUCATI   EDMA_RD0 EDMA_RD1 EDMA_RD2 EDMA_RD3 EDMA_WR0 \n");
  dprintf(" %-8d %-8d %-8d %-8d %-8d %-8d %-8d %-8d\n",
  ...
  dprintf(" \n");
  dprintf(" EDMA_WR1 EDMA_WR2 EDMA_WR3 SGX      HDVICP0  HDVICP1  HDVICP2  GMAC0    \n");
  dprintf(" %-8d %-8d %-8d %-8d %-8d %-8d %-8d %-8d\n",
  ...
  dprintf(" \n");
  dprintf(" GMAC1    USB_DMA  USB_QMGR SATA     PCIE\n");
  dprintf(" %-8d %-8d %-8d %-8d %-8d\n",
  ...
  dprintf(" \n");
  dprintf(" L3 Bandwith Regulator \n");
  dprintf(" ===================== \n");
  dprintf(" Module   : PressH   PressL   Bandwidth in MB/s Watermark (Cycles) \n");
  for(i=0; i<BW_REG_MAX; i++)
  {
