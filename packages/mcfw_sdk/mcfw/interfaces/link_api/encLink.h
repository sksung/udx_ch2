...
#define ENC_LINK_CMD_SET_CODEC_PACKETSIZE  		(0x5012)
#define ENC_LINK_CMD_SET_CODEC_ROI          	(0x5013)
#define ENC_LINK_CMD_SET_CODEC_MDPRM			(0x5014)
#define ENC_LINK_CMD_SET_CODEC_INPUTKEY     	(0x5015)
#define ENC_LINK_CMD_SET_CODEC_ENCPRESET   		(0x5016)
#define ENC_LINK_CMD_SET_CODEC_RATECONTROL  	(0x5017)

#define ENC_LINK_CMD_SET_VA_METADATA			(0x5018)
...
/**
    \brief Enc link channel rate control set config params

    Defines encoder rateControl parameters that can be changed dynamically
    on a per channel basis for the encode link
*/
typedef struct EncLink_ChRateControlParams {
    UInt32 chId;
    /**< Encoder channel number */
    UInt32 rateControl;
    /**< Modified encoder rate control value */
} EncLink_ChRateControlParams;
...
/**
    \brief Enc link channel packet size set config params

    Defines encoder PacketSize parameters that can be changed dynamically
    on a per channel basis for the encode link
*/
typedef struct EncLink_ChPacketSizeParams {
    UInt32 chId;
    /**< Encoder channel number */
    UInt32 packetSize;
    /**< Encoder channel number */
} EncLink_ChPacketSizeParams;
...
/**
    \brief Enc link channel InputKey set config params

    Defines encoder InputKey parameters that can be changed dynamically
    on a per channel basis for the encode link
*/
typedef struct EncLink_ChInputKeyParams {
    UInt32 chId;
    UInt32 inputKey;
    /**< Encoder channel number */
} EncLink_ChInputKeyParams;

/**
    \brief Enc link channel Encode Preset set config params

    Defines encoder Encode Preset parameters that can be changed
    on a per channel basis for the encode link
*/
typedef struct EncLink_ChEncodePresetParams {
    UInt32 chId;
    /**< Encoder channel number */
    UInt32 encodePreset;

    UInt32 bMotionEnable;
    UInt32 bMotionCEnale;
    UInt32 MotionLevel;
    UInt32 MotionCValue;
    UInt32 MotionBlock;
    /**< Motion Detection params */

} EncLink_ChEncodePresetParams;
...
typedef struct EncLink_ChCreateParams
{
    ...
    Int32 StreamPreset;
    /**< Flag Identifying which preset is set   */
...
typedef struct
{
    ...
    UInt32 vsEnable;
    /**< flag indicate if VS is enabled*/

    UInt32 isVaUseCase;
    /**< flag indicate if current use case is VA */
