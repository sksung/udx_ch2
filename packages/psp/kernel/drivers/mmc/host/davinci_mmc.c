...
#ifdef CONFIG_MACH_DAVINCI_DM368_CVR
...
/* MMC_CTLR_VERSION_2 -> 512bit FIFIO (64byte) */
static unsigned rw_threshold = 64; //#32;
		...
		struct mmc_command *cmd)
{
	...
	/*
	 * poll status for small size transfers.
	 * For small size non-dma sdio transactions.
	 */
...
static void mmc_davinci_dma_cb(unsigned channel, u16 ch_status, void *data)
{
	if (DMA_COMPLETE != ch_status) {
		...
		if (!(host->data)) {
			dev_warn(mmc_dev(host->mmc), "DMA Event Miss / NULL Transfr\n");
 ...
 */
static irqreturn_t mmc_davinci_detect(int irq, void *dev_id)
{
	struct mmc_davinci_host *host = dev_id;
	...
	carddetect = config->get_cd(pdev->id);
	...
	if (carddetect) {
		...
		ext_report_event(EV_KEY, KEY_F7, 1);
		ext_report_event(EV_KEY, KEY_F7, 0);
	} else {
		ext_report_event(EV_KEY, KEY_F8, 1);
		ext_report_event(EV_KEY, KEY_F8, 0);
...
static int __init davinci_mmcsd_probe(struct platform_device *pdev)
{
	...
	/* allow driver to work without DMA resource */
...
#ifdef CONFIG_MACH_DAVINCI_DM368_CVR
	ret = gpio_request(host->gpio_cd, "mmc_cd");
	if (ret) {
		dev_err(mmc_dev(host->mmc), "Failed to request mmc cd gpio!\n");
		goto free_irq;
	}
...
#ifdef CONFIG_MACH_DAVINCI_DM368_CVR
	ret = request_threaded_irq(gpio_to_irq(host->gpio_cd),
					   			NULL,
					   			mmc_davinci_detect,
					   			IRQF_TRIGGER_RISING | IRQF_TRIGGER_FALLING | IRQF_ONESHOT,
					   			mmc_hostname(host->mmc), host);
	if (ret) {
		dev_err(mmc_dev(host->mmc), "Unable to grab MMC CD IRQ\n");
		goto free_gpio;
	}
#endif
...
#ifdef CONFIG_MACH_DAVINCI_DM368_CVR
		free_irq(gpio_to_irq(host->gpio_cd), host);
		gpio_free(host->gpio_cd);
#endif
