/*
 *	adc101.c - lm_sensors driver for adc101c021/adc101c027 10-bit ADC
...
#include <linux/adc101.h>
#include <linux/miscdevice.h>
#include <linux/uaccess.h>
#include <linux/mutex.h>

#define	DRIVER_NAME					"adc101c"

#define ADC_RESOL			1024
#define VREF_MV 			3300 /* Internal vref is 3.3V, 3300mV */

/* The ADC101C registers (Pointer Address) */
...
static struct miscdevice adc101c_misc_device;

static void adc101c_work_func(struct work_struct *work)
{
	struct adc101_data *data =
		container_of(work, struct adc101_data, work.work);
...
static irqreturn_t adc101_isr(int irq, void *dev)
{
	struct adc101_data *data = (struct adc101_data *)dev;
...
static int adc101c_misc_open(struct inode *inode, struct file *file)
{
	struct adc101_data *pos, *data = NULL;
	unsigned short min;
	
	list_for_each_entry(pos, &adc_data_list, list) {
		if (adc101c_misc_device.minor == iminor(inode)) {
			data = pos;
			break;
		}
	}
...
static int adc101c_misc_release(struct inode *inode, struct file *filp)
{
	struct adc101_data *data = filp->private_data;
...
static ssize_t adc101c_misc_read(struct file *file, char __user *buf,
				size_t count, loff_t *pos)
{
	struct adc101_data *data = file->private_data;
	struct i2c_client *client = data->client;
...
static long adc101c_misc_ioctl(struct file *file, unsigned int cmd,
			   unsigned long arg)
{
	struct adc101_data *data = file->private_data;
	int val, ret = 0;
...
static int adc101c_misc_fasync(int fd, struct file *file, int on)
{
	struct adc101_data *data = file->private_data;
...
static unsigned int adc101c_misc_poll(struct file *file, poll_table *wait)
{
	struct adc101_data *data = file->private_data;
...
static const struct file_operations adc101c_misc_fops = {
	.owner   = THIS_MODULE,
	.llseek  = no_llseek,
	.read    = adc101c_misc_read,
	.open    = adc101c_misc_open,
	.release = adc101c_misc_release,
	.fasync  = adc101c_misc_fasync,
	.poll    = adc101c_misc_poll,
	.unlocked_ioctl = adc101c_misc_ioctl,
};

static struct miscdevice adc101c_misc_device = {
	.minor   = MISC_DYNAMIC_MINOR,
	.name    = "adc101",
	.fops    = &adc101c_misc_fops,
};

static int adc101c_probe(struct i2c_client *client,
			 const struct i2c_device_id *id)
{
	struct device *dev = &client->dev;
	struct i2c_adapter *adapter = client->adapter;
	const struct adc101_platform_data *pdata = client->dev.platform_data;
	...
	data = kzalloc(sizeof(struct adc101_data), GFP_KERNEL);
	if (!data) {
	...
	INIT_DELAYED_WORK(&data->work, adc101c_work_func);	
	data->client = client;
	
	if (client->irq) {
		err = request_threaded_irq(client->irq, NULL, 
					   adc101_isr, IRQF_TRIGGER_FALLING | IRQF_ONESHOT,
		   			   "adc101-irq", data);
		if (err) {
	...
	err = misc_register(&adc101c_misc_device);
	if (err) {		
...
static int adc101c_remove(struct i2c_client *client)
{
	struct adc101_data *data = i2c_get_clientdata(client);
	
	misc_deregister(&adc101c_misc_device);
	if (client->irq) {
		free_irq(client->irq, data);
	}
...
static const struct i2c_device_id adc101c_id[] = {
	{ "adc101c", 0 },
	{ }
};
MODULE_DEVICE_TABLE(i2c, adc101c_id);

static struct i2c_driver adc101c_driver = {
	.driver = {
		.name = DRIVER_NAME,
		.owner	= THIS_MODULE,
	},
	.probe = adc101c_probe,
	.remove = adc101c_remove,
	.id_table = adc101c_id,
};

static int __init adc101c_init(void)
{
	return i2c_add_driver(&adc101c_driver);
}
module_init(adc101c_init);

static void __exit adc101c_exit(void)
{
	i2c_del_driver(&adc101c_driver);
}
module_exit(adc101c_exit);

MODULE_AUTHOR("UDWORKs");
MODULE_DESCRIPTION("ADC101C driver");
MODULE_LICENSE("GPL");
