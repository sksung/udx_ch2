...
static int usbfs_fill_super(struct super_block *sb, void *data, int silent)
{
	...
	if (!inode) {
		printk(KERN_ERR "%s: could not get inode!",__func__);
		return -ENOMEM;
	}

	root = d_alloc_root(inode);
	if (!root) {
		printk(KERN_ERR "%s: could not get root dentry!",__func__);
		iput(inode);
		return -ENOMEM;
	}
	sb->s_root = root;
			      ...
			      struct dentry *parent, struct dentry **dentry)
{
	...
	if (!parent) {
		printk(KERN_ERR "Ah! can not find a parent!");
		return -EFAULT;
	}
				      ...
				      uid_t uid, gid_t gid)
{
	...
	printk(KERN_DEBUG "creating file '%s'",name);
