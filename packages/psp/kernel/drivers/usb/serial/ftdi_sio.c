...
static void ftdi_set_max_packet_size(struct usb_serial_port *port)
{
	...
	dev_dbg(&udev->dev, "Number of endpoints %d\n", num_endpoints);
	...
	for (i = 0; i < num_endpoints; i++) {
		dev_dbg(&udev->dev, "Endpoint %d MaxPacketSize %d\n", i+1,
			interface->cur_altsetting->endpoint[i].desc.wMaxPacketSize);
		ep_desc = &interface->cur_altsetting->endpoint[i].desc;
		if (ep_desc->wMaxPacketSize == 0) {
			ep_desc->wMaxPacketSize = cpu_to_le16(0x40);
			dev_dbg(&udev->dev, "Overriding wMaxPacketSize on endpoint %d\n", i);
		}
	}
	...
	dev_dbg(&udev->dev, "Setting MaxPacketSize %d\n", priv->max_packet_size);
}
...
static int ftdi_NDI_device_setup(struct usb_serial *serial)
{
	...
	dev_dbg(&udev->dev, "NDI device with a latency value of %d", latency);
