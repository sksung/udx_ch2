...
static int bma150_open(struct bma150_data *bma150)
{
	...
	if (error && error != -ENOSYS) {
		dev_err(&bma150->client->dev, "pm_runtime_get_sync error\n");
		return error;
	}
	...
	if (bma150->mode != BMA150_MODE_NORMAL) {
		...
		if (error) {
			dev_err(&bma150->client->dev, "bma150_set_mode error\n");
			return error;
		}
	}
	
	dev_dbg(&bma150->client->dev, "bma150 open succeed!!\n");
	return 0;
}
						...
						struct input_dev *idev)
{
	...
	idev->phys = BMA150_DRIVER "/input1";
				  ...
				  const struct i2c_device_id *id)
{
	...
	if (client->irq > 0) {
		...
		dev_dbg(&client->dev, "bma150 irq request succeed %d\n",
				client->irq);
	} 
	else {
		...
		dev_dbg(&client->dev, "bma150 configure poll mode\n");
	}
