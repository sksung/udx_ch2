...
static struct vps_dcvencinfo venc_info = {
	{
		{VPS_DC_VENC_HDMI,
			...
			148500, 60, 88, 148, 44, 4, 36, 5}, 3, 0, 0},

		#ifdef CONFIG_MACH_IMK
		{VPS_DC_VENC_DVO2,
		    {FVID2_STD_HVGA_60, 320, 480, FVID2_SF_PROGRESSIVE,\
			12500, 60, 10, 8, 10, 4, 4, 10}, 1, 6, 0},
		#else
		{VPS_DC_VENC_DVO2,
		    {FVID2_STD_WVGA_60, 480, 800, FVID2_SF_PROGRESSIVE,\
			/*30000, 60, 164, 89, 10, 10, 23, 10}, 1, 6, 0},*/
			28000, 60, 20, 20, 4, 16, 15, 10}, 1, 6, 0},
		#endif

		{VPS_DC_VENC_SD,
		    {FVID2_STD_NTSC, 720, 480, FVID2_SF_INTERLACED,\
			216000, 60, 12, 68, 64, 5, 41, 5}, 0, 0, 0},

		{VPS_DC_VENC_HDCOMP,
		    {FVID2_STD_1080I_60, 1920, 1080, FVID2_SF_PROGRESSIVE,\
			74250, 60, 88, 148, 44, 2, 15, 5}, 2, 0, 0}
	},
	0,
	VPS_DC_MAX_VENC,
};
			  ...
			  const char *clksrc)
{
	...
	//disp_ctrl->blenders[HDMI].mask = 0;
	//disp_ctrl->blenders[HDCOMP].mask = 0;
	//# to do mask for ubx
	#endif
	...
	#if !defined(CONFIG_COMMON_UD_FEATURE)		//# use mcfw default
	/*config the PLL*/
	for (i = 0; i < VPS_DC_MAX_VENC; i++) {
		if (!disp_ctrl->blenders[i].mask)
			continue;
		...
		if (r) {
			VPSSERR("failed to set pll %d(%d)", venc_info.modeinfo[i].minfo.pixelclock, i);
			goto cleanup;
		}
	}
	#endif
