...
static int io_init(struct ubi_device *ubi)
{
	...
	dbg_gen("physical eraseblock size:   %d bytes (%d KiB)",
		ubi->peb_size, ubi->peb_size >> 10);
	dbg_gen("logical eraseblock size:    %d bytes", ubi->leb_size);
	dbg_gen("smallest flash I/O unit:    %d", ubi->min_io_size);
	if (ubi->hdrs_min_io_size != ubi->min_io_size)
		dbg_gen("sub-page size:              %d",
			ubi->hdrs_min_io_size);
	dbg_gen("VID header offset:          %d (aligned %d)",
		ubi->vid_hdr_offset, ubi->vid_hdr_aloffset);
	dbg_gen("data offset:                %d", ubi->leb_start);
