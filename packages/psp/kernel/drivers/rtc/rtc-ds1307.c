...
#	define MCP7941X_BIT_ST		0x80 /* on-chip osc start & stop */
...
#	define MCP7941X_BIT_VBATEN	0x08 /* VBATEN */
...
static s32 ds1307_native_smbus_write_block_data(struct i2c_client *client,
...
static s32 ds1307_native_smbus_read_block_data(struct i2c_client *client,
...
read_rtc:
	...
	if (chip->nvram_size) {
		ds1307->nvram = kzalloc(sizeof(struct bin_attribute), GFP_KERNEL);
		if (!ds1307->nvram) {
			err = -ENOMEM;
			goto err_irq;
		}

		ds1307->nvram->attr.name = "rtcram";
