...
/* from fat.c */
extern __u8 do_fat_read_at_block[MAX_CLUSTSIZE];
extern __u8 get_contents_vfatname_block[MAX_CLUSTSIZE];
extern __u8 get_dentfromdir_block[MAX_CLUSTSIZE];

extern block_dev_desc_t *cur_dev;
extern disk_partition_t cur_part_info;
extern const int vfat_enabled;

extern int read_bootsectandvi(boot_sector *bs,
					volume_info *volinfo, int *fatsize);
extern int disk_read(__u32 block, __u32 nr_blocks, void *buf);
extern __u8 mkcksum(const char name[8], const char ext[3]);
extern int get_cluster(fsdata *mydata, __u32 clustnum,
