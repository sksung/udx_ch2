...
#if defined(COMMON_UD_FEATURE) && !defined(CONFIG_ISPN)
	u32 slave_port_num;
#endif
...
#if defined(COMMON_UD_FEATURE) && !defined(CONFIG_ISPN)
	int slave_port = priv->slave_port_num;
	struct cpsw_slave_data	*data = &priv->data.slave_data[slave_port] + slave_num;
	slave_num = slave_port;
#else
...
#if defined(COMMON_UD_FEATURE) && !defined(CONFIG_ISPN)
	priv->slave_port_num = data->slave_port_num;
#endif
