...
{
	char uart_lcr=UART_LCR_8N1;
	if(com_port == CONFIG_SYS_NS16550_COM3) {
		uart_lcr = UART_LCR_8E1;
	}
...
#endif
	serial_out(UART_LCR_BKSE | uart_lcr, (ulong)&com_port->lcr);
	serial_out(0, &com_port->dll);
	serial_out(0, &com_port->dlm);
	serial_out(uart_lcr, &com_port->lcr);
	serial_out(UART_MCRVAL, &com_port->mcr);
	serial_out(UART_FCRVAL, &com_port->fcr);
	serial_out(UART_LCR_BKSE | uart_lcr, &com_port->lcr);
