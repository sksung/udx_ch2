...
static uint omap24_i2c_setspeed(uint speed)
{
	...
	if (speed >= OMAP_I2C_HIGH_SPEED) {
		...
		fsscll = I2C_INTERNAL_SAMPLING_CLK / (2 * OMAP_I2C_FAST_MODE);
		fssclh = fsscll;
...
void i2c_init (int speed, int slaveadd)
{
	...
	if (0 != omap24_i2c_setspeed(speed)) {
...
int i2c_probe(uchar chip)
{
	...
	if (!(status & I2C_STAT_NACK)) {
		res = 0;				/* Device found */
		udelay(1000);/* Required by AM335X in SPL */
