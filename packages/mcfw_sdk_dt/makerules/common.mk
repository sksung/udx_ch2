...
include $(MCFW_ROOT_PATH)/makerules/build_config.mk
include $(MCFW_ROOT_PATH)/makerules/env.mk
include $(MCFW_ROOT_PATH)/makerules/platform.mk
...
ifeq ($(DEST_ROOT),)
 ifdef MODULE_NAME
  OBJDIR = build/obj/$(DEPENDENCY_SUB_PATH)/$(PROFILE_$(CORE))
  LIBDIR = build/lib/$(DEPENDENCY_SUB_PATH)/$(PROFILE_$(CORE))
 else
  OBJDIR = build/obj/$(PLATFORM)/$(CORE)/$(PROFILE_$(CORE))
  BINDIR = bin/$(PLATFORM)
 endif
else
  ifdef MODULE_NAME
...
PKG_LIBS = $(foreach LIB,$(PKG_LIST),$(DEST_ROOT)/$($(LIB)_RELPATH)/lib/$($(LIB)_DEPSUBPATH)/$(PROFILE_$(CORE))/*)
PKG_OBJS = $(foreach LIB,$(PKG_LIST),$(DEST_ROOT)/$($(LIB)_RELPATH)/obj/$($(LIB)_DEPSUBPATH)/$(PROFILE_$(CORE))/*)
endif
...
#   eg: rules_m3.mk
include $(MCFW_ROOT_PATH)/makerules/rules_$(ISA).mk
...
clean :
	$(RM) -rf $(CONFIGURO_DIR)
	$(RM) -f $(OBJDIR)/* $(DEPDIR)/* $(PKG_LIBS) $(PKG_OBJS)
