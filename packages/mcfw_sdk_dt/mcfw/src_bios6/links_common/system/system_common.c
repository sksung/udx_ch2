...
{
	#ifdef SYSTEM_DEBUG_MEM_PRINT
	Vps_printf(" ==================== System memory heap status ========================\r\n");
    Vps_printf
            (" SYSTEM : FREE Space : System Heap      = %d B, Mbx = %d msgs) \r\n",
    ...
    if(System_getSelfProcId()==SYSTEM_PROC_M3VPSS)
    {
...

        {
            ...
            {
                Vps_printf
                    (" SYSTEM : FREE Space : Tiler Buffer     = %d B (%d MB) - TILER OFF \r\n",
                        Utils_getCurTimeInMsec(),
            ...
            {
                Vps_printf
                    (" SYSTEM : FREE Space : Tiler 8-bit      = %d B (%d MB) - TILER ON \r\n",
                        tilerFreeSize.freeSize8b,
                        tilerFreeSize.freeSize8b/(1024*1024) );
                Vps_printf
                    (" SYSTEM : FREE Space : Tiler 16-bit     = %d B (%d MB) - TILER ON \r\n",
