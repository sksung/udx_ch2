...
static Int32 VnfLink_drvInitCh(VnfLink_Obj * pObj)
{
    ...
    for (chId = 0; chId < pObj->inQueInfo.numCh; chId++)
    {
        ...
        /**Make it FALSE if we don't want to enable stage A NSF***/
        chVnfCreateParams->nsfParams.bSpatialFilterEnableForStageA = (MSP_BOOL)TRUE;
   if(chCreateParams->sParams.eOperateMode == VNF_LINK_3DNF)
        {   
            if(chVnfCreateParams->nsfParams.bSpatialFilterEnableForStageA)
            {
    ...
    else    if (chVnfCreateParams->eOperateMode != MODE_TNF)
        {
            chCreateParams->sParams.computeWidth = inChInfo->width - 32;
            chCreateParams->sParams.computeHeight = inChInfo->height - 32;
        ...
        if (chVnfCreateParams->eOperateMode != MODE_TNF)
            VnfChObj->format.height = chVnfCreateParams->ulComputeHeight + 32;
        else
            VnfChObj->format.height = chVnfCreateParams->ulComputeHeight;
         ...
         */
        Utils_memFrameAlloc(&VnfChObj->format, VnfChObj->frames,
       ...
       if (chVnfCreateParams->eOperateMode != MODE_TNF)
         {
          if(chVnfCreateParams->nsfParams.bSpatialFilterEnableForStageA == (MSP_BOOL)FALSE && chVnfCreateParams->eOperateMode == VNF_LINK_3DNF)
           outChInfo->startY = 0;
          else
           outChInfo->startY = 32;
         }  
...
Int32 VnfLink_drvStop(VnfLink_Obj * pObj)
{
    ...
    VNFLINK_INFO_LOG("Stop in progress !!!\n");
...
Int32 VnfLink_drvDelete(VnfLink_Obj * pObj)
{
    ...
    for (chId = 0; chId < pObj->inQueInfo.numCh; chId++)
    {
        ...
        Utils_memFrameFree(&vnfChObj->format, vnfChObj->frames,
                           VNF_LINK_MAX_OUT_FRAMES_PER_CH);
...
Int32 VnfLink_drvEnableLDC(VnfLink_Obj * pObj, UInt32 enableLDC)
{
    ...
    VNFLINK_INFO_LOG("EnableLDC mode %d LDC %d!!!\n",
                     !pObj->createArgs.chCreateParams[0].sParams.eOperateMode,
                     enableLDC);
