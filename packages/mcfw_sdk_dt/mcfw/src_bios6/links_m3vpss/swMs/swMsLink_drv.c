...
Int32 SwMsLink_drvPrintLayoutParams(SwMsLink_Obj * pObj)
{
    ...
    Vps_printf( " \n"
            " *** [%s] Mosaic Parameters *** \n"
            " Output FPS: %d\n"
            " Win| Ch | Input       | Output     | Output      | Input       | Data   |\n"
            " Num| Num| Wi x He     | Start X, Y | Wi x He     | Memory Type | Format |\n"
            " -------------------------------------------------------------------------\n",
            pObj->name, pObj->layoutParams.outputFPS
	);
    ...
    for (winId = 0; winId < pObj->layoutParams.numWin; winId++)
    {
        ...
        if(pWinObj->scRtInFrmPrm.dataFormat==FVID2_DF_YUV422I_YUYV)
            strcpy(strDataFormat, "422I ");
        else if(pWinObj->scRtInFrmPrm.dataFormat==FVID2_DF_YUV420SP_UV)
            strcpy(strDataFormat, "420SP");
        else
            strcpy(strDataFormat, "UNKNWN");

        Vps_printf (" %2d | %2d | %4d x %4d | %4d, %4d | %4d x %4d | %s | %6s |\n",
                winId,
                chNum,
                pWinObj->scRtInFrmPrm.width,
                pWinObj->scRtInFrmPrm.height,
...
Int32 SwMsLink_drvGetInputChInfoFromWinId(SwMsLink_Obj * pObj,SwMsLink_WinInfo * pCropInfo)
{
    ...
    #else	//# this is window size
    pCropInfo->startX   = pWinObj->scRtCropCfg.cropStartX;
    pCropInfo->startY   = pWinObj->scRtCropCfg.cropStartY;
    pCropInfo->width    = pWinObj->scRtInFrmPrm.width;
    pCropInfo->height   = pWinObj->scRtInFrmPrm.height;
    #endif
...
Int32 SwMsLink_drvSetScCoeffs (FVID2_Handle fvidHandle, Bool loadUpsampleCoeffs, Bool isDei)
{
    Int32 retVal = FVID2_SOK;
	...
	#if 0
    Vps_rprintf(" %d: SWMS: %s Co-effs Loading ... DONE !!!\n",
    	Utils_getCurTimeInMsec(), loadUpsampleCoeffs?"Up-scaling":"Down-scaling");
    #endif
...
Int32 SwMsLink_drvCreate(SwMsLink_Obj * pObj, SwMsLink_CreateParams * pPrm)
{
    ...
    #if SWMS_LINK_USE_TIMER	//# why use timer?
...
Int32 SwMsLink_drvProcessData(SwMsLink_Obj * pObj)
{
    ...
    if (frameList.numFrames)
    {
    	//Vps_printf("--- %s numFrames:%d\n", __func__, frameList.numFrames);
        ...
        for (frameId = 0; frameId < frameList.numFrames; frameId++)
        {
			...
			#if 0	//# phoong, if progress, not used
...
Int32 SwMsLink_drvMakeFrameLists(SwMsLink_Obj * pObj, FVID2_Frame * pOutFrame)
{
    ...
    for (winId = 0; winId < maxChnl; winId++)
    {
        ...
        {
            /* empty input queue */
            status = Utils_bufGetFullFrame(&pWinObj->bufInQue, &pInFrame, BIOS_NO_WAIT);
            if (status == FVID2_SOK)
            {
                pWinObj->framesRejectCount++ ;
        ...
        if(pWinInfo->channelNum == SYSTEM_SW_MS_INVALID_ID)
        {
            ...
            {
                pWinObj->framesRejectCount++ ;
                SwMsLink_freeFrame(pInQueParams, &pObj->freeFrameList, pWinObj->pCurInFrame);
                pWinObj->pCurInFrame = NULL;
            }
        }
        ...
        pDrvObj->inFrameList.frames[pDrvObj->inFrameList.numFrames] = pInFrame;
        pDrvObj->inFrameList.numFrames++;
        pDrvObj->outFrameList.frames[pDrvObj->outFrameList.numFrames] = &pWinObj->curOutFrame;
        ...
        if (pFrameInfo != NULL)
        {
            if (pFrameInfo->rtChInfoUpdate == TRUE)
            {
                ...
                // Checks for pitch 1 also but we are passing 0 for pitch 1.So commenting pitch 1 check
                if (pFrameInfo->rtChInfo.pitch[0] != rtChannelInfo->pitch[0])
                {
                    rtChannelInfo->pitch[0] = pFrameInfo->rtChInfo.pitch[0];
					rtChannelInfo->pitch[1] = pFrameInfo->rtChInfo.pitch[0];
                    rtParamUpdatePerFrame = TRUE;
                }
                                 ...
                                 SwMsLink_LayoutPrm * layoutParams)
{
	...
	if (layoutParams->outputFPS == 0 || layoutParams->outputFPS > 200) {
		...
		pObj->timerPeriod = (1000/(layoutParams->outputFPS+(layoutParams->outputFPS/10)));
	}
	#else
	//# use default period
	pObj->timerPeriod = SW_MS_LINK_TIMER_DEFAULT_PERIOD;
