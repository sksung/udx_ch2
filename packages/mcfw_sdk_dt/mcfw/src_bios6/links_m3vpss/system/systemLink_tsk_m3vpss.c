...
Int32 SystemLink_cmdHandler(SystemLink_Obj *pObj, UInt32 cmd, Void *pPrm)
{
    ...
    switch (cmd)
    {
		...
		case SYSTEM_M3VPSS_CMD_CMD_SYNC_DATETIME:
		{
			UInt_64 *prm = (UInt_64 *) pPrm;
			Utils_set_walltime(*prm);
			break;
		}
        ...
        {
            SystemCommon_TilerAlloc *tilerAlloc = (SystemCommon_TilerAlloc *) pPrm;
            UInt32 utilsTilerCntMode =
                (tilerAlloc->cntMode - SYSTEM_TILER_CNT_FIRST) + UTILS_TILER_CNT_FIRST;

            if (((Int32) utilsTilerCntMode >= UTILS_TILER_CNT_FIRST)
                	&& (utilsTilerCntMode <= UTILS_TILER_CNT_LAST))
            {
                tilerAlloc->tileAddr = Utils_tilerAllocatorAlloc(utilsTilerCntMode,
                                              tilerAlloc->width, tilerAlloc->height);
                if (UTILS_TILER_INVALID_ADDR == tilerAlloc->tileAddr) {
        ...
        {
            SystemCommon_TilerAllocRaw *tilerRawAlloc = (SystemCommon_TilerAllocRaw *) pPrm;

            tilerRawAlloc->allocAddr  =
            (UInt32) Utils_tilerAllocatorAllocRaw(tilerRawAlloc->size, tilerRawAlloc->align);
            break;
        }
        case SYSTEM_COMMON_CMD_TILER_FREE_RAW:
        {
            SystemCommon_TilerFreeRaw *tilerRawFree = (SystemCommon_TilerFreeRaw *) pPrm;

            Utils_tilerAllocatorFreeRaw((Ptr)tilerRawFree->allocAddr, tilerRawFree->size);
        ...
        case SYSTEM_M3VPSS_CMD_GET_DEVICE_STATUS:
        {
        	status = system_getDeviceStatus((VSYS_M3VPSSDEV_S *) pPrm);
        	break;
        }
