...
static Int32 DecLink_mpeg4DecoderFlush(DecLink_MPEG4Obj * hObj);
                ...
                if(outArgs->viddec3OutArgs.extendedError)
                {
                    ...
                    needReset =
                      DecLink_mpeg4DecoderResetCheck(outArgs->viddec3OutArgs.
                                                     extendedError);
                    status = DecLink_mpeg4DecoderFlush(hObj);
                    if (status == XDM_EOK)
                    {
                        inArgs->viddec3InArgs.numBytes = pReqObj->InBuf->fillLength;
...
*/
    ...
    UTILS_MEMLOG_PRINT(" DECLINK_MPEG4", hObj->memUsed, (sizeof(hObj->memUsed) / sizeof(hObj->memUsed[0])));
