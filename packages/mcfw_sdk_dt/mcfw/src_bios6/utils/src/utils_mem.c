...
Int32 Utils_memFrameAlloc_capture(FVID2_Format * pFormat,
                                  FVID2_Frame * pFrame, UInt16 numFrames,
                                  Int32 Padding, Int32 BoxCarDataSize)
{
    UInt32 size, cOffset, frameId;
    ...
    if (status == 0)
    {
        ...
        /* for all 'numFrames' memory is contigously allocated */
        pBaseAddr =
            Utils_memAlloc((size + Padding ) * numFrames, VPS_BUFFER_ALIGNMENT);
		if(BoxCarDataSize != 0)
			pBaseAddrBC = Utils_memAlloc_cached(( BoxCarDataSize) * numFrames, 4*1024);
                                 ...
                                 Int32 Padding, Int32 BoxCarDataSize)
{
    ...
    if (status == 0)
    {
		...
		if(BoxCarDataSize != 0)
			Utils_memFree_cached(pFrame->blankData, (  BoxCarDataSize) * numFrames);
    }
...
Ptr Utils_memAlloc(UInt32 size, UInt32 align)
{
    ...
    Vps_printf("MEM: Alloc'ing from FRAME BUF. (required size = %d B, free space = %d B)\n",
        size, Utils_memGetBufferHeapFreeSpace() );
    #endif
    ...
    if(addr==NULL)
    {
        ...
        */
        if (SystemTiler_isAllocatorDisabled())
        {
            ...
            Vps_printf("MEM: Alloc'ing from TILER BUF. (required size = %d B, free space = %d B)\n",
                size,
        ...
        {
            #ifdef UTILS_MEM_DEBUG
            Vps_printf("MEM: TILER is ENABLED, will try BITS BUF (required size = %d B)\n", size);
            #endif
        }
        ...
        {
            #ifdef UTILS_MEM_DEBUG
            Vps_printf("MEM: Alloc'ing from BITS BUF. (required size = %d B, free space = %d B)\n",
                size, Utils_memGetBitBufferHeapFreeSpace() );
            #endif
    ...
    #if UTILS_MEM_DEBUG_MIN//def UTILS_MEM_DEBUG
    Vps_printf("--- MEM: FRAME ALLOC, addr = 0x%08x, size = %d B, free space = %d B \n",
    	addr, size, Utils_memGetBufferHeapFreeSpace());
    #endif
...
Ptr Utils_memAlloc_cached(UInt32 size, UInt32 align)
{
    ...
    #ifdef UTILS_MEM_DEBUG
    Vps_printf("MEM: FRAME ALLOC, addr = 0x%08x, size = %d bytes \n",
                addr, size
               );
    #endif
...
Int32 Utils_memFree(Ptr addr, UInt32 size)
{
    ...
    if((UInt32)addr >= (UInt32)srEntry.base && (UInt32)addr < ((UInt32)srEntry.base + srEntry.len) )
    {
        ...
        #ifdef UTILS_MEM_DEBUG
        Vps_printf("MEM: FRAME FREE, addr = 0x%08x, size = %d bytes, heap = [FRAME BUF]\n", addr, size);
        #endif
    ...
    else
    {
        ...
        if((UInt32)addr >= (UInt32)srEntry.base && (UInt32)addr < ((UInt32)srEntry.base + srEntry.len) )
        {
            ...
            #ifdef UTILS_MEM_DEBUG
            Vps_printf("MEM: FRAME FREE, addr = 0x%08x, size = %d bytes, heap = [BITS BUF]\n", addr, size);
            #endif
        ...
        {
            #ifdef UTILS_MEM_DEBUG
            Vps_printf("MEM: FRAME FREE, addr = 0x%08x, size = %d bytes, heap = [TILER BUF]\n", addr, size);
            #endif
    ...
    #if 0
    Vps_printf("--- MEM: FRAME FREE, addr = 0x%08x, size = %d B\n", addr, size);
    #endif

    return 0;
}

Int32 Utils_memFree_cached(Ptr addr, UInt32 size)
{
    SharedRegion_Entry srEntry;
...
Int32 Utils_memBitBufAlloc(Bitstream_Buf * pBuf, UInt32 size, UInt16 numFrames)
{
    ...
    #if UTILS_MEM_DEBUG_MIN//def UTILS_MEM_DEBUG
    Vps_printf("MEM: BITS ALLOC, addr = 0x%08x, size = %d (%dx%d) bytes\n", pBaseAddr, (size * numFrames), size, numFrames);
    #endif
...
Int32 Utils_memBitBufFree(Bitstream_Buf * pBuf, UInt16 numFrames)
{
    ...
    #ifdef UTILS_MEM_DEBUG
    Vps_printf("MEM: BITS FREE, addr = 0x%08x, size = %d bytes\n", pBuf->addr, (size * numFrames));
    #endif
