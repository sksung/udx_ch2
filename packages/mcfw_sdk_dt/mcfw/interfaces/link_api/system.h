...
/**
    ...
    The IPNC RDK is a Reference Design Kit for TI812x platform and is optimized
    for IPNC multi-resolution applications. RDK allows a user to create different
    multi-resolution data flows involving video camera, capture,
    video processing (VS, Noise Filter, Encode, LDC etc) and video display.
    The IPNC-RDK is implemented using TI's Multi-Channel "Link" FrameWork (McFW).
    ...
    \par Multi-Channel FrameWork (McFW) API
     ...
     <b> The user API for using IPNC-RDK is via the Multi-Channel FrameWork (McFW) API. </b>

     Multi-Channel FrameWork (McFW) API allows user to setup
     and control pre-defined application specific chains for IPNC, DVR, using a
	 ...
	 - \ref MCFW_VCAM_API for Video Camera Configuration and Control
