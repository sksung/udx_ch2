                           ...
                           IpcBitsInLinkHLOS_CreateParams * pPrm)
{
    ...
    status = System_ipcListMPOpen(pObj->createArgs.baseCreateParams.inQueParams.
    ...
    while(!ListMP_empty(pObj->listMPInHndl))
    {
        ...
        if (listElem != NULL)
        {
            IPCBITSINLINK_INFO_LOG(pObj->tskId,
                                   "InList not empty!!! Stale entry:[%p]",
                                   listElem);
        }
    }
                                     ...
                                     Bitstream_Buf ** pBitBufPtr)
{
    ...
    OSA_assert(SharedRegion_isCacheEnabled(SharedRegion_getId(pListElem)) == FALSE);
...
Int32 IpcBitsInLink_processBitBufs(IpcBitsInLink_Obj * pObj)
{
    ...
    while (1)
    {
        ...
        OSA_assert(SYSTEM_IPC_BITS_GET_BUFSTATE(pListElem->bufState) == IPC_BITBUF_STATE_OUTQUE);

        SYSTEM_IPC_BITS_SET_BUFOWNERPROCID(pListElem->bufState);
        SYSTEM_IPC_BITS_SET_BUFSTATE(pListElem->bufState, IPC_BITBUF_STATE_DEQUEUED);

        //pBitBuf->reserved[0] = curTime;
        pObj->stats.recvCount++;
        status = OSA_quePut(&pObj->outBitBufQue, (Int32) pBitBuf, OSA_TIMEOUT_NONE);
        OSA_assert(status == OSA_SOK);
                                    ...
                                    Bitstream_BufList * pBitBufList)
{
    ...
    if (pBitBufList->numBufs)
    {
        ...
        for (bufId = 0; bufId < pBitBufList->numBufs; bufId++)
        {
            ...
            SYSTEM_IPC_BITS_SET_BUFSTATE(pListElem->bufState, IPC_BITBUF_STATE_INQUE);
        ...
        if (pObj->createArgs.baseCreateParams.notifyPrevLink) {
            System_ipcSendNotify(pObj->createArgs.baseCreateParams.inQueParams.prevLinkId);
        }
    }
