...
Uint64 OSA_getCurTimeInUsec(void)
{
	static int isInit = FALSE;
	static Uint64 initTime=0;
	struct timeval tv;
	...
	if(isInit==FALSE)
	{
		...
		initTime = (Uint64)(tv.tv_sec * 1000000u + tv.tv_usec);
	}

	if (gettimeofday(&tv, NULL) < 0)
		return 0;

	return (Uint64)(tv.tv_sec * 1000000u + tv.tv_usec)-initTime;
}
