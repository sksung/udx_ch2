...
static char* find_ext(void)
{
	...
	sprintf(path, "%s/*%s", DIR_DISK, F_EXT);
...
void delete_nand_update()
{
    ...
    if(del==0){
        ...
        sprintf(cmd, "rm -f %s/%s", DIR_DISK, F_EMERGENCY);
        dev_execlp(cmd);

        sprintf(cmd, "rm -f %s/%s", DIR_DISK, F_MLO);
        dev_execlp(cmd);

        sprintf(cmd, "rm -f %s/%s", DIR_DISK, F_RFS);
        dev_execlp(cmd);

        sprintf(cmd, "rm -f %s/%s", DIR_DISK, F_UBOOTBIN);
        dev_execlp(cmd);

        sprintf(cmd, "rm -f %s/%s", DIR_DISK, F_UBOOTNAND);
        dev_execlp(cmd);

        //sprintf(cmd, "rm -f %s/%s", DIR_DISK, F_LOGO);
        //dev_execlp(cmd);

        sprintf(cmd, "rm -f %s/%s", DIR_DISK, F_UIMAGE);
        dev_execlp(cmd);
...
int check_fw()
{
...
#if 0
	//# check older f/w
	sprintf(buf, "%s.%s.%s", fw[FW_MAJOR].value, fw[FW_MINOR].value, fw[FW_PATCH].value);
	...
	if(del_cfg){
		...
		//# backup cfg for fw update.
		ubx_write_setting();
	}


	sprintf(buf, "%s/%s", DIR_DISK, F_EMERGENCY);
	if (0 == access(buf, 0)) {
		/* debugging */
...
int ctrl_wifi_set(int on)
{
    ...
    if (on)
    {
    	sprintf( msg, "[APP_CTRL] WiFi ON ssid:%s, pwd:%s", app_set->net.ssid, app_set->net.pwd);
		app_log_write( MSG_LOG_WRITE, msg );
...
void app_ctrl_out_copy(void)
{
	...
	sprintf(path, "/mmc/%s_app.out", "d16");

	if(0 == access(path, 0)) // existence only
	{
		sprintf(cmd, "cp %s /opt/%s/bin/.", path, "d16");
		dev_execlp(cmd);
