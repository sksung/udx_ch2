...
void app_snd_init(void)
{
	...
	//dev_snd_set_output_path(SND_SPK1_OUT);
//	dev_snd_set_volume(SND_VOLUME_P, get_volum(app_set->aud.out_vol));
	...
	OSA_thrCreate(&tObj->thr, sound_main, FXN_TSK_PRI, 0, tObj);
	isnd->sObj = tObj;
}

void app_snd_exit(void)
{
	app_thr_obj *tObj = (app_thr_obj *)isnd->sObj;
