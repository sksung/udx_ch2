...
static FILE *avi_file_open(char *full_path, char* fname, stream_info_t *ifr, int mode)
{
	...
	ftime(&t_start);
	favi = LIBAVI_createAvi(full_path, &aviInfo, 1);
	ftime(&t_end);
...
static int nor_file_open(stream_info_t *ifr)
{
...
#ifdef DTC_BIKE		//# record per 10 min
    if(!irec->rec_first){
        if(ts.tm_min%10 == 0)
            ten_minute_change = 1;
    }
    ...
    if(irec->rec_first){
 	...
 	if( (irec->old_min != ts.tm_min && ten_minute_change) || irec->chg_file || app_set->rec_type == REC_TYPE_MOT)//irec->evt_rec)
	{
#else
