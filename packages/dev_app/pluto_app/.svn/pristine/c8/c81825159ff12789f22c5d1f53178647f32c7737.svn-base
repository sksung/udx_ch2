...
/*----------------------------------------------------------------------------
 check voltage
-----------------------------------------------------------------------------*/
static int volt_level_chk = 1, volt_peek_cnt=0, volt_skip_cnt=0;
static int dev_check_volt(int volt)
{
	...
	if(volt < VOLT_LOW)
	{
        ...
        if(volt_peek_cnt >= CNT_PEEK_VOLT_CHK && imicom->low_pwr_detect == 0)
        {
			...
			ctrl_system_shutdown(SYS_OFF_LOWPWR);
...
//			printf(" [app] >>>>>>>> EXIT Peek Low Volt(%d.%dV) <<<<<<< \n", (imicom->val.svolt/100), (imicom->val.svolt%100) );
	...
	if( (app_cfg->state.acc == 0) || (app_cfg->state.park) )
	{
		...
		if(volt_skip_cnt > CNT_VOLT_SKIP && imicom->low_pwr_detect == 0)
		{
	        app_cfg->state.pwroff = 1;
            imicom->low_pwr_detect = 1;
			
			sprintf( msg, ">>>> Low voltage Time out (%d.%dV) <<<<", (volt/100), (volt%100) );
			app_log_write( MSG_LOG_SHUTDOWN, msg );
			printf("%s \n", msg);
			
			ctrl_system_shutdown(SYS_OFF_LOWBATT);
		    return 1;
		}		
	}
	...
	if(app_cfg->state.cap)
	{
	    ...
	    if(temp >= app_cfg->temp_high)
	    {
	        ...
	        if(temp_cnt > CNT_TEMP_CHK)
	        {
                ...
                ctrl_system_shutdown(SYS_OFF_HITEMP);
				return 1;
	        }
	    }
	    else
	...
	if(acc_evt_on)
	{
		...
		if(pre_acc_val == acc)
		{
			...
			if(acc_cnt >= CNT_MODE_CHANGE)
			{
				...
				{
					app_cfg->state.acc = 1;
					app_dev_set_shutdown_timer(OFF);
				...
				else if(acc == ACC_OFF)
				{
					...
					if( app_set->prec_info.rec_mode == PREC_MODE_NOT_USE)
					{
						sprintf(msg, " >>>>> Parking mode not used - Shut donw!! <<<<<");
						app_log_write(MSG_LOG_SHUTDOWN, msg);
						printf("%s\n", msg);
						ctrl_system_shutdown(SYS_OFF_TIMER);
					} 
					else if(app_set->prec_info.rec_mode == PREC_MODE_EVT)
					{
						ctrl_mode_change(MODE_PARKING);
					}

					app_dev_set_shutdown_timer(ON);
				}
			}
		}
	}
