...
//				printf("mmc errors repaired!\n");
	...
	app_cfg->state.hdd = 0;
	if(USE_HDD_REC){
	    app_cfg->state.hdd = dev_disk_check_mount_ex(HDD_MOUNT_POINT, app_cfg->hdd_mount_path);
	}

    //if(sd_res && app_cfg->state.mmc){
	...
	//# start log system
	app_log_start();
	app_log_write(MSG_LOG_WRITE, ">>>>>>> 2CH HD USB2 CW START!! <<<<<<<");
	
	sprintf(msg, "--- S/W:%s%s MIC:%s ---", TCX_MODEL, TCX_SW_VER, app_cfg->micom_ver);
	app_log_write(MSG_LOG_WRITE, msg);
    ...
    if(dev_fw_printenv("nand_update")==2){	//# done firmware updated.
    	app_log_write( MSG_LOG_WRITE, "[UCX_MAIN] Firmware update done....");
        check_nand_update();
        dev_fw_setenv("nand_update", "0", 0);
    }
    else{	//# detecting firmware file...
        if(check_fw() != SOK){
            check_nand_update();
        }
    }
