...
typedef struct{
	...
	unsigned int shutdown_cyle_time;
} app_dev_t;
...
static void *timer_main(void *prm)
{
	...
	while (!exit)
	{
		...
		//------------------------------------//
		//#--- shutdown timer for parking mode
		//------------------------------------//
		if(idev->shutdown_timer)
		{
		 	if(idev->shutdown_time == idev->shutdown_cyle_time)
		 	{
				sprintf(msg, ">>>>>> Reached the SHUT-DOWN time!!! - System shut donw. <<<<<<");
				app_log_write(MSG_LOG_SHUTDOWN, msg);
				printf("%s\n", msg);

				ctrl_system_shutdown(SYS_OFF_TIMER);
	 		}
			
			idev->shutdown_cyle_time += TIMER_INTERVAL;			
		}
...
void app_dev_set_shutdown_timer(int on)
{
	...
	{
		//# ALWAYS PARKING MODE
		if(app_set->prec_info.rec_mode == PREC_MODE_EVT && app_set->prec_info.run_time == PREC_RUNTIME_0){
			printf(">>>> It is Always event!!! <<<< \n");
			return;
		}
	
		idev->shutdown_time = DEV_1_MINUTE*get_prec_run_time(app_set->prec_info.rec_mode, app_set->prec_info.run_time);
		idev->shutdown_cyle_time = 0;
		
		printf(">>>> Parking mode Timer ON [%d]msec <<<< \n", idev->shutdown_time);
		printf("");
	}

	idev->shutdown_timer = on;
}
