...
static int PREC_RUNTIME[PREC_MODE_MAX][PREC_RUNTIME_MAX] = {
	{30, 60, 120, 440},	//# normal rec
	{ 0, 60, 180, 720}, //# event rec
	{ 0,  0,   0,   0}	//# not use parking mode
...
static int REC_DUR[REC_DUR_MAX]				= {5, 10, 20};

static char mmc_cfg[CHAR_64];
static char hdd_cfg_dir[CHAR_64];
...
app_set_t app_default_set = {
	...
	.wd = {
		...
		.temp      = TEMP_IDX_4,	//# temparature
		.volt      = VOLT_IDX_7,	//# minimum voltage
	...
	.sys = {
		...
		.rec_duration = REC_DUR_10,
	},
	.prec_info = {
		.rec_mode = PREC_MODE_NOT_USE,
		.run_time = PREC_RUNTIME_0,
	}
};
...
int get_prec_run_time(int rec_type, int time_idx)
{
	return PREC_RUNTIME[rec_type][time_idx];
}
...
int get_rec_dur_val(app_rec_dur_e idx)
{
	...
	if(idx >= REC_DUR_05 && idx < REC_DUR_MAX)
		dur_min = REC_DUR[idx];
...
static void cfg_param_check(app_set_t* pset)
{
	...
	if(pset->sys.rec_duration < REC_DUR_05 || pset->sys.rec_duration >= REC_DUR_MAX) {
		pset->sys.rec_duration = REC_DUR_10;
	}

	if(pset->prec_info.rec_mode < PREC_MODE_NOR || pset->prec_info.rec_mode >= PREC_MODE_MAX){
		pset->prec_info.rec_mode = PREC_MODE_NOT_USE;
	}

	if(pset->prec_info.run_time < PREC_RUNTIME_0|| pset->prec_info.run_time >= PREC_RUNTIME_MAX){
		pset->prec_info.run_time = PREC_RUNTIME_0;
	}
}
...
static int get_cfg_hdd(void)
{
    ...
    if(-1 == access(hdd_cfg_dir, 0)) {
        mkdir(hdd_cfg_dir, 0775);
        chmod(hdd_cfg_dir, 0775);
		return EFAIL;
    }
    ...
    if(cfg_size != setSize){
        eprintf("\n ### CFG size defferent in HDD(file:%d/cfg:%d) - Set Default ### \n", cfg_size, sizeof(app_set_t));
		ubx_set_default();
    }
	else {
...
void ubx_delete_cfg(void)
{
	...
	if(app_cfg->state.hdd){
		sprintf(cmd, "rm -Rf %s", hdd_cfg_dir);
		dev_execlp(cmd);
	}
...
int ubx_open_setting(void)
{
	...
	sprintf(hdd_cfg_dir, "%s/%s", app_cfg->hdd_mount_path, CFG_DIR);
	
    sprintf(mmc_cfg,  "%s/%s%s", CFG_DIR_MMC,  model, CFG_INI_PREFIX);	
    sprintf(hdd_cfg,  "%s/%s%s", hdd_cfg_dir,  model, CFG_INI_PREFIX);
