...
static int log_open(int new_file)
{
	...
	sprintf(ilog->last_fname, "%s_%08x.%s", LOG_PREFIX, ilog->last_idx, LOG_EXT);
	sprintf(path, "%s/%s", ilog->log_path, ilog->last_fname);
...
static void *log_write_main(void *prm)
{
    ...
    while(!exit)
    {
		...
		{
			case MSG_LOG_WRITE:
				if(ilog->fd != NULL && !app_cfg->state.mmc_err)
				{
					//# Write Log
					fprintf(ilog->fd, "%s\r\n", msg_buf.msgstr);
...
static int log_list_check(char*filters)
{
    ...
    if(-1 == access(ilog->log_path, 0)) {
        mkdir(ilog->log_path, 0775);
        chmod(ilog->log_path, 0775);

		return 0;
    }

	//# get count of log files.
	fcnt = get_log_count(ilog->log_path, filters);

	if(fcnt > 0)
	{
		scan[0] = ilog->log_path;
		...
		{
			printf("file : %s\n", list[i].fname);
			sprintf(path, "%s/%s", ilog->log_path, list[i].fname);
			log_file_add( path );
		}
...
int app_log_start(void)
{
	...
	if( !app_cfg->state.mmc && !app_cfg->state.hdd )
		return EFAIL;
	...
	{
		sprintf( ilog->log_path, "%s/%s", app_cfg->hdd_mount_path, LOG_EXT);
	}
	else if(app_cfg->state.mmc)
	{
		sprintf( ilog->log_path, "%s", LOG_ROOT_PATH );
	}
