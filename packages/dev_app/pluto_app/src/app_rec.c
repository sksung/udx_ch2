...
typedef struct {
	...
	int rec_min;		//# rec duration 5,10,20
} app_rec_t;
...
static void avi_file_close(FILE *favi, char* fname, int rec_type)
{
	...
	if(favi) {
		...
		app_file_update(fname, 1, rec_type);
	}
}
...
#else
	if( (irec->old_min != ts.tm_min && minute_change) || irec->chg_file || irec->evt_rec)
#endif 	
	{
...
//        if(app_set->rec_type == REC_TYPE_NOR)
		if(!irec->evt_rec && !app_cfg->state.park)
        {
			...
			app_file_update(irec->nor_fname, 0, REC_DRIVE);

			if(app_get_dir_name(rec_dir, REC_DRIVE) != 0){
...
static int evt_file_open(stream_info_t *ifr)
{
	...
	strftime(buf, sizeof(buf), "%Y%2m%2d_%2H%2M%2S", &ts);
	sprintf(irec->evt_fname, "E_%s_%dch.%s", buf, irec->num_ch,REC_EXT);
	
	app_file_update(irec->evt_fname, 0, REC_EVENT);

	if(app_get_dir_name(rec_dir, REC_EVENT) != 0 ){
		eprintf("Failed Get Rec Dir!!! \n");
		return -1;
    }
...
//					app_log_write( MSG_LOG_WRITE, msg );
		...
		avi_file_close(irec->fevt, irec->evt_fname, REC_EVENT);
		irec->fevt = NULL;
