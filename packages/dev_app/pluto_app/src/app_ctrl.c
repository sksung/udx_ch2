...
int ctrl_mode_change(int mode)
{
	...
	if(mode == MODE_PARKING)
	{
		if(!app_cfg->state.park)
		{
			...
			ctrl_enc_motion_set(0, app_set->wd.motion);
			ctrl_enc_motion_set(1, app_set->wd.motion);
			change = 1;
 		}
	}
	else
	{
		if(app_cfg->state.park)
		{
			ctrl_enc_motion_set(0, 0);
			ctrl_enc_motion_set(1, 0);		
    ...
    if (on)
    {
        ...
        if (app_cfg->state.wifi == 0) {
			...
			if (!ret) {
				 ...
				 * 3th -> Wi-Fi Channel
				 * 4th -> 0:2.4GHz 1:5GHz.
				 * 5th -> stealth on/off -> default off.
				 */
...
{
	int micom_exit_code = OFF_LOWPWR;
	int micom_start_con = START_VOLTAGE_09V;

	if(off_type == SYS_OFF_LOWPWR || off_type == SYS_OFF_LOWBATT)			//# low voltage
	{
		ctrl_tvo_enable(OFF);
		ctrl_hdcam_pwroff();
	}
	
	if(off_type == SYS_OFF_HITEMP)		//# high temp detect
	{
		micom_exit_code = OFF_HITEMP;
		micom_start_con = START_TEMERATURE;
	}
	else if(off_type == SYS_OFF_LOWBATT || off_type == SYS_OFF_NORMAL)
	{
		micom_exit_code = OFF_NORMAL;
		micom_start_con = 0;
	}

	app_rec_stop();
	app_file_stop();
	if(off_type != SYS_OFF_LOWPWR) ubx_write_setting(0);

	mic_exit_state(micom_exit_code, micom_start_con);
	system_buzzer(SYSTEM_ONOFF);
	delay_3sec_exit();
}
