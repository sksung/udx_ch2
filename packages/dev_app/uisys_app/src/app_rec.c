...
#define TIME_PRE_REC_10MIN	600 //# 10min
...
static void check_rec_dir()
{
	...
	if(-1 == access(DIR_REC_DRIVE, 0)) {
		mkdir(DIR_REC_DRIVE, 0775);
		chmod(DIR_REC_DRIVE, 0775);
	}
	if(-1 == access(DIR_REC_EVENT, 0)) {
...
-----------------------------------------------------------------------------*/
static FILE *avi_file_open(char *file_name, stream_info_t *ifr, int mode)
{
	FILE *favi;
	...
	ftime(&t_start);
	favi = LIBAVI_createAvi(file_name, &aviInfo, 0);
	ftime(&t_end);
...
static int nor_file_open(stream_info_t *ifr)
{
	...
	localtime_r((const time_t *)&ifr->t_sec, &ts);
 	if( (irec->old_min != ts.tm_min) || irec->chg_file || irec->evt_rec)
	{
...
//		irec->num_ch 	= app_cfg->num_ch;
		...
		if(! irec->evt_rec && !app_cfg->state.park && (app_set->sys.motion_only == OFF) )
        {
            memset(irec->nor_fname, 0, CHAR_128);

            strftime(buf, sizeof(buf), "%Y%2m%2d_%2H%2M%2S", &ts);
            sprintf(irec->nor_fname, "D_%s_%dch.avi", buf, app_cfg->num_ch);
			sprintf(full_path, "%s/%s", DIR_REC_DRIVE, irec->nor_fname);

			printf("new filename %s\n", full_path);

			if(app_cfg->state.mmc && !app_cfg->state.mmc_err)
			{
				irec->favi = avi_file_open(full_path, ifr, app_cfg->num_ch-1);
				if(irec->favi == NULL){
...
static int nor_file_save(stream_info_t *ifr)
{
	...
	if(ifr->ch > app_cfg->num_ch-1)		//# for sync app_cfg->num_ch
		return 0;
...
//                sprintf( msg, " !!! nor_file_save() failed !!!" );
//				app_log_write( MSG_LOG_WRITE, msg );
...
static int evt_file_open(stream_info_t *ifr)
{
	...
	irec->fevt = avi_file_open(full_path, ifr, app_cfg->num_ch-1);
...
//				app_log_write( MSG_LOG_WRITE, msg );
		...
		while(!done)
		{
			//# wait cmd
			//cmd = event_wait(tObj);
			if(tObj->cmd == UBX_CMD_STOP || app_cfg->num_ch == 0) {
				break;
			}
...
//	irec->num_ch 	= app_cfg->num_ch;
	...
	//# record directory init ---
	check_rec_dir(0);		//# 1: delete all data
...
*	- desc
*****************************************************************************/
void app_evt_record(int rec_type)
{
	if(!app_cfg->state.rec || irec==NULL || !app_cfg->state.mmc || app_cfg->state.mmc_err || app_cfg->num_ch == 0)
		return;

	if(irec->evt_rec == 0) {
		irec->evt_rec 		= 1;
		irec->evt_type 		= rec_type;
		app_cfg->state.evt 	= 1;
...
int get_evt_type(void)
{
	int rec_type = REC_DRIVE;

	if(irec)
		rec_type = irec->evt_type;
	return rec_type;
}
