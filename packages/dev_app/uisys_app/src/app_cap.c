			...
			if(pFullBuf->codecType == IVIDEO_H264HP && pFullBuf->chnId < ENC_D1_CHANNEL)
			{
                ...
                if(pFullBuf->chnId == ch && pFullBuf->frameType == VCODEC_FRAME_TYPE_I_FRAME)
                {
                    ...
                    if(addr != NULL)
                    {
                        ...
                        app_dev_get_gs((void*)&gmeta.gsensor);
...
static void capt_enc_late_init(void)
{
	...
	for(i=0; i<MAX_CH_NUM; i++)
	{
		...
		//#--- apply motion
        if(app_cfg->state.park || app_set->sys.motion_only)
...
int app_capt_start()
{
	...
	#if 0
	//#--- parking mode setting on system start if acc is off
	if(!app_cfg->state.park && dev_dio_get_value(DIO_ACC_DET) == ACC_OFF)	//# 0: acc-on, 1: acc-off
	{
