    ...
    if(app_cfg->state.sd_buzzer || app_cfg->state.cam_buzzer){
		...
		{ 
			...
			{ 
                ...
                if(app_set->aud.snd_on && !app_cfg->state.pwroff)
...
static void get_wd_name(char* name)
{
	...
	if( !(app_cfg->wd_flags & WD_GSN ) )
		strcat(name, WD_GSN_NAME);
...
static void *gui_main(void *prm)
{
	...
	while(!exit)
	{
		...
		//# watchdog
		if((wd_cycle != 0) && (wd_cycle%UI_WATCHDOG_TIME == 0) && !app_cfg->state.pwroff)
		{
		    if(app_cfg->num_ch == 0)
...
int app_capt_main(void)
{
    ...
    mic_send_msg(CMD_BUZZER, 1, BUZZER_200);
	
	app_log_write(MSG_LOG_WRITE, "[APP_GUI] app_capt_main() started");
...
void ui_draw_txt_tvo(void)
{
	...
	str_h 	= draw_font_height( TCX_SW_VER, FNT_SZ01 );	
    ...
    if(app_cfg->state.ui_cap){
        ...
        if(app_cfg->state.wifi){
            draw_text(igui->gfx_obj.tvo.buf, app_set->net.ssid, FNT_SZ01, 210, TVO_GAP_Y, RGB_WHITE, RGB_BLACK);
        }
		//# GPS
		st.x = 210 + draw_string_len( app_set->net.ssid, FNT_SZ01 ) + 10;
        ...
        if(app_cfg->tvo_flag & TVO_GSENSOR){
            st.x += draw_string_len(formVolt, FNT_SZ01) + SPACE_W;
            if(app_set->wd.gsn == GSN_IDX_OFF)
                sprintf(item, "S-0");
            else
                sprintf(item, "S-%d", app_set->wd.gsn+1);
            if(app_cfg->state.shock)
                draw_text(igui->gfx_obj.tvo.buf, item, FNT_SZ01, st.x, st.y, RGB_RED, RGB_BLACK);
            else
                draw_text(igui->gfx_obj.tvo.buf, item, FNT_SZ01, st.x, st.y, RGB_WHITE, RGB_BLACK);
        }

        if(app_cfg->tvo_flag & TVO_MOTION){
            st.x += draw_string_len(formGlv, FNT_SZ01) + SPACE_W;
            sprintf(item, "M-%d", app_set->wd.motion);
            if(app_cfg->state.motion)
                draw_text(igui->gfx_obj.tvo.buf, item, FNT_SZ01, st.x, st.y, RGB_RED, RGB_BLACK);
            else
                draw_text(igui->gfx_obj.tvo.buf, item, FNT_SZ01, st.x, st.y, RGB_WHITE, RGB_BLACK);
        }
        ...
        if(app_cfg->tvo_flag & TVO_MIC){
            st.x += draw_string_len(formMlv, FNT_SZ01) + SPACE_W;
