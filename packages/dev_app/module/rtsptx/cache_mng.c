...
void CacheMng_cache_reset( CACHE_DATA *pcache)
{
	...
	if( pcache )
	{
		...
		eprintf("CacheMng_cache_reset : NULL \n");
	}
}
...
void CacheMng_blk_unset( CACHE_BLK *pblk)
{
	if( pblk )
	{
		...
		eprintf("CacheMng_blk_unset : NULL \n");
	}
}
...
void CacheMng_blk_reset( CACHE_BLK *pblk, int offset)
{
	if( pblk )
	{
		...
		eprintf("CacheMng_blk_reset : NULL \n");
	}
}
...
int CacheMng_Init(MEM_MNG_INFO *pInfo)
{
	...
	for( cnt = 0; cnt < VIDOE_INFO_END ; cnt++ )
	{
		if( Cache_mng_default[cnt].video_type == pInfo->video_info[cnt].video_type )
		{
			...
			{
				eprintf("CacheMng_Init malloc FAIL \n");
					return CACHE_PARM_ERR;
			}
...
int CacheMng_GetCacheInfoBySerial( int serial, CACHE_DATA_INFO *pCache_info, VIDEO_BLK_INFO *pVidInfo )
{
	...
	if( findcache == 1 )
	{
		...
		return pCache->offset;//pVidInfo->frame[cacheindex].offset;
	}
	else
	{
		return 0;
...
int CacheMng_cache_write( CACHE_DATA *pCache, VIDEO_FRAME *pFrame, VIDEO_BLK_INFO *pVidInfo )
{
	...
	{
		eprintf("CacheMng_cache_write: invalidate parameter!! \n");
		return CACHE_PARM_ERR;
	}
	...
	if( CacheMng_MemCpy( (void *)Des_Addr, (void *)Src_Addr, size ) < 0 ) {
		eprintf("CacheMng_cache_write: error memcpy \n");
		return CACHE_PARM_ERR;
	}
	#endif
...
int CacheMng_Video_CacheLock( int serial , VIDEO_BLK_INFO *pVidInfo )
{
	...
	if( pFrame )
	{
		...
		if( blk_index < 0 ) {
			eprintf("pFrame->realsize = %d \n",pFrame->realsize);
			eprintf("CacheMng_Find_Freeblk Fail bkl_sz[%d] blk_use[%d] blk_num[%d]\n", blk_sz, blk_use, pVidInfo->cachemng->blk_num);
			ret = CACHE_NOMEM;
			goto CACHE_FAIL;
		}
		...
		if( cache_index < 0 ) {
			eprintf("CacheMng_Find_FreeCache Fail \n");
			ret = CACHE_NOMEM;
			goto CACHE_FAIL;
		}
...
CACHE_FAIL:
	eprintf("CacheMng_Video_Cache: CACHE_FAIL \n");
	return ret;
}
								...
								int serial , VIDEO_BLK_INFO *pVidInfo )
{
	...
	if( !pVidInfo || serial<0 )
	{
		eprintf("CacheMng_Video_CacheRead: error parameter \n");
		return CACHE_PARM_ERR;
	}
	...
	if( pCache == NULL )
	{
		eprintf("CacheMng_Video_CacheRead: can not serial \n");
		return CACHE_PARM_ERR;
	}

	if( pCache->realsize > bufflimit )
	{
		eprintf("CacheMng_Video_CacheRead: over bufflimit \n");
...
int CacheMng_Video_CacheUnlock( int serial , VIDEO_BLK_INFO *pVidInfo )
{
	...
	if( !pVidInfo || serial<0 )
	{
		eprintf("CacheMng_Video_Cache: error parameter \n");
		return CACHE_PARM_ERR;
	}
	//ShowLockInfo();
	...
	if( cache_index < 0 )
	{
		eprintf("CacheMng_Video_Free: search cache fail!! \n");
		return CACHE_ERR;
	}
	pCache = &(pVidInfo->cachemng->cache[cache_index]);
