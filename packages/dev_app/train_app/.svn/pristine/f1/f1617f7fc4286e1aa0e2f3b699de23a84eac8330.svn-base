...
const int LOGLISTRES_SIZE = sizeof(COMRESPACKET) ;
...
void connect_user(int channel, char* data, int len)
{
    CONNECT_USER *Connect_user ;
    DEVICERES Connect_user_res ;    

    int user_lv = 0, sendlen = 0 ;
    char msg[CHAR_128] = {0,} ;

    Connect_user = (CONNECT_USER *)data ;
    user_lv = ntohs(Connect_user->level) ;

    switch(user_lv)
    {
        case 0:
           
           sprintf( msg, " YEC Account login !!");
           app_log_write( MSG_LOG_WRITE, msg );
           break ;
        case 1:
           
           sprintf( msg, "  Inspector Account login !!");
           app_log_write( MSG_LOG_WRITE, msg );
           break ;
        case 2:
           
           sprintf( msg, " Train Crew Account login !!");
           app_log_write( MSG_LOG_WRITE, msg );
           break ;
    }

    Connect_user_res.identifier = ntohs(IDENTIFIER) ;
    Connect_user_res.cmd = ntohs(CMD_CONNECT_USER_RES) ;
    Connect_user_res.length = ntohs(8) ;
    Connect_user_res.result = ntohs(1) ;

    memcpy(m_SendBuffer[channel], &Connect_user_res, DEVICERES_SIZE) ;
    sendlen = send(SystemInfo.Channel[channel], m_SendBuffer[channel], DEVICERES_SIZE, 0) ;
    memset(m_SendBuffer[channel], MAXBUFF, 0) ;
    
#ifdef NETWORK_DEBUG
    DEBUG_PRI("Connect_user_res packet sendlen = %d\n",sendlen) ;
#endif
    
}

void recdurparamreq(int channel, char *data, int len)
{
    RECDURPARAM_RES Recdurparam_res ;
    int sendlen = 0 ;

    Recdurparam_res.identifier = htons(IDENTIFIER) ;
    Recdurparam_res.cmd = htons(CMD_RECDURPARAM_RES) ;
    Recdurparam_res.length = htons(DEVICERES_SIZE) ;
    Recdurparam_res.dur = htons(app_set->sys.rec_duration) ;

    memcpy(m_SendBuffer[channel], &Recdurparam_res, DEVICERES_SIZE) ;
    sendlen = send(SystemInfo.Channel[channel], m_SendBuffer[channel], DEVICERES_SIZE, 0) ;
    memset(m_SendBuffer[channel], MAXBUFF, 0) ;
#ifdef NETWORK_DEBUG
    DEBUG_PRI("Recdur_paramres packet sendlen = %d\n",sendlen) ;
#endif

}

void recdurcfgreq(int channel, char *data, int len)
{
    RECDURCFG_REQ *Recduration ;
    DEVICERES Recdu_res ;

    int dur, sendlen ;

    Recduration = (RECDURCFG_REQ *)data ;
 
    dur = ntohs(Recduration->dur) ;
    printf("Recduration->dur = %d\n",dur) ;
    app_rec_change_dur(dur) ;

    Recdu_res.identifier = htons(IDENTIFIER) ;
    Recdu_res.cmd = htons(CMD_RECDURCFG_RES) ;
    Recdu_res.length = htons(DEVICERES_SIZE) ;
    Recdu_res.result = htons(TRUE) ;

    memcpy(m_SendBuffer[channel], &Recdu_res, DEVICERES_SIZE) ;
    sendlen = send(SystemInfo.Channel[channel], m_SendBuffer[channel], DEVICERES_SIZE, 0) ;
    memset(m_SendBuffer[channel], MAXBUFF, 0) ;
#ifdef NETWORK_DEBUG
    DEBUG_PRI("Recdu_cfgres packet sendlen = %d\n",sendlen) ;
#endif
}
...
void playlistparamreq(int channel, char *data, int len)
{
	...
	cnt = app_file_get_list(&pinfo, ON);;
    if(ntohs(Playlistparamreq->rec_type) > REC_PARKING)
        cnt = 0;
...
void qualityparamreq(int channel, char *data, int len)
{
    ...
    Qualityparamres.Rec_bitrate      = htons(app_set->ch[CH_TYPE_RECORDING].quality) ; 
    Qualityparamres.Rec_framerate    = htons(app_set->ch[CH_TYPE_RECORDING].framerate) ;
    Qualityparamres.Live_bitrate      = htons(app_set->ch[CH_TYPE_STREAMING].quality) ;
    Qualityparamres.Live_framerate    = htons(app_set->ch[CH_TYPE_STREAMING].framerate) ;
...
void infoparamreq(int channel, char *data, int len)
{
    ...
    strncpy(Infoparamres.HWver, app_set->info.hw_ver, STRING_SIZE) ;
    strncpy(Infoparamres.SWver, app_set->info.sw_ver, STRING_SIZE) ;
    strncpy(Infoparamres.Product, app_set->info.model, STRING_SIZE) ;
    strncpy(Infoparamres.ModelNo, app_set->info.pname, STRING_SIZE) ;
    strncpy(Infoparamres.SSID, app_set->wifiap.ssid, STRING_SIZE) ;
    strncpy(Infoparamres.Passwd, app_set->wifiap.pwd, CHAR_16) ;
...
void displaycfgreq(int channel, char *data, int len)
{
	...
	if(MIRROR_MAX > ntohs(Displaycfgreq->mirroring)
		...
		ctrl_hdcam_mirror(0, app_set->lcd.sub_mirror);
        ubx_write_setting() ;
        sync() ;
	}
...
void qualitycfgreq(int channel, char *data, int len)
{
    ...
    retval = ctrl_set_enc_prm(CH_TYPE_RECORDING, ntohs(Qualitycfgreq->Rec_framerate), ntohs(Qualitycfgreq->Rec_bitrate)) ;
    retval &= ctrl_set_enc_prm(CH_TYPE_STREAMING, ntohs(Qualitycfgreq->Live_framerate), ntohs(Qualitycfgreq->Live_bitrate)) ;
...
void systemcfgreq(int channel, char *data, int len)
{
    ...
    if(ntohs(Systemcfgreq->fwupdate) < 2){
    	//retval = ctrl_upgrade();
		//if(retval == SOK)
			//dev_system_reboot();
   	}

    if(ntohs(Systemcfgreq->initial) < 2)
    {
	    ubx_set_default();
	    OSA_waitMsecs(100);
...
void sdstatusreq(int channel, char *data, int len)
{
#ifdef NETWORK_DEBUG
    DEBUG_PRI("sdstatusreq packet receive...\n") ;
#endif
    int sendlen = 0, retval = EFAIL, ret = 0 ;

    COMRESPACKET Sdstatusres ;

    Sdstatusres.identifier = htons(IDENTIFIER) ;
    Sdstatusres.cmd        = htons(CMD_SDSTATUS_RES) ;
    Sdstatusres.length     = htons(SDCFGRES_SIZE) ;

    if(app_cfg->state.mmc && !app_cfg->state.mmc_err)
    	Sdstatusres.result     = htons(CFGRES_OK) ;
    else
    	Sdstatusres.result     = htons(CFGRES_ERR_SYSTEM) ;

    memcpy(m_SendBuffer[channel], &Sdstatusres, SDCFGRES_SIZE) ;

    sendlen = send(SystemInfo.Channel[channel], m_SendBuffer[channel], SDCFGRES_SIZE, 0) ;


#ifdef NETWORK_DEBUG
    DEBUG_PRI("sdstatusres.. packet sendlen = %d\n",sendlen) ;
#endif
}

void connect_info_req(int channel, char *data, int len)
{

}

void camstatusreq(int channel, char *data, int len)
{
#ifdef NETWORK_DEBUG
    DEBUG_PRI("camstatusreq packet receive...\n") ;
#endif
    int sendlen = 0, retval = EFAIL, ret = 0 ;

    COMRESPACKET Camstatusres ;

    Camstatusres.identifier = htons(IDENTIFIER) ;
    Camstatusres.cmd        = htons(CMD_CAMSTATUS_RES) ;
    Camstatusres.length     = htons(SDCFGRES_SIZE) ;

    Camstatusres.result     = htons(!app_cfg->state.cam_buzzer) ;

    memcpy(m_SendBuffer[channel], &Camstatusres, SDCFGRES_SIZE) ;

    sendlen = send(SystemInfo.Channel[channel], m_SendBuffer[channel], SDCFGRES_SIZE, 0) ;


#ifdef NETWORK_DEBUG
    DEBUG_PRI("camstatusres.. packet sendlen = %d\n",sendlen) ;
#endif
...
void wificfgreq(int channel, char *data, int len)
{
	...
	if(((Wificfgreq->SSID != NULL) && strlen(Wificfgreq->SSID) < CHAR_32)
		&& (strcmp(Wificfgreq->SSID, app_set->wifiap.ssid)!=0)){
		sprintf(app_set->wifiap.ssid,"%s", Wificfgreq->SSID);
        retssid = SOK ;
	}
    else
    {
	...
	if((strlen(Wificfgreq->PASSWD) < CHAR_16 && strlen(Wificfgreq->PASSWD) >= CHAR_8)
		&& (strcmp(Wificfgreq->PASSWD, app_set->wifiap.pwd)!=0)){
		sprintf(app_set->wifiap.pwd, "%s", Wificfgreq->PASSWD);
        retpasswd = SOK ;
	}
    else
    {
...
static void decodingHdr(AVIHeadBlock* pHdr)
{
	AVIHeadBlock copyHdr;
	memcpy(&copyHdr, pHdr, sizeof(AVIHeadBlock));

	pHdr->ck_riff.ckID 		= ~(copyHdr.ck_riff.ckID);
	pHdr->ck_riff.ckSize 	= ~(copyHdr.ck_riff.ckSize);
	pHdr->ck_riff.ckCodec 	= ~(copyHdr.ck_riff.ckCodec);
	pHdr->ck_hdrl.ckID 		= ~(copyHdr.ck_hdrl.ckID);
	pHdr->ck_hdrl.ckSize 	= ~(copyHdr.ck_hdrl.ckSize);
	pHdr->ck_hdrl.ckType 	= ~(copyHdr.ck_hdrl.ckType);
	pHdr->ck_avih.ckID 		= ~(copyHdr.ck_avih.ckID);
	pHdr->ck_avih.ckSize 	= ~(copyHdr.ck_avih.ckSize);

	pHdr->avih.dwMicroSecPerFrame 		= ~(copyHdr.avih.dwMicroSecPerFrame);
	pHdr->avih.dwMaxBytesPerSec			= ~(copyHdr.avih.dwMaxBytesPerSec);
	pHdr->avih.dwFlags 					= ~(copyHdr.avih.dwFlags);
	pHdr->avih.dwTotalFrames			= ~(copyHdr.avih.dwTotalFrames);
	pHdr->avih.dwInitialFrames			= ~(copyHdr.avih.dwInitialFrames);
	pHdr->avih.dwStreams				= ~(copyHdr.avih.dwStreams);
	pHdr->avih.dwSuggestedBufferSize 	= ~(copyHdr.avih.dwSuggestedBufferSize);
	pHdr->avih.dwWidth					= ~(copyHdr.avih.dwWidth);
	pHdr->avih.dwHeight 				= ~(copyHdr.avih.dwHeight);
	pHdr->avih.dwRate 					= ~(copyHdr.avih.dwRate);
	pHdr->avih.dwScale 					= ~(copyHdr.avih.dwScale);

	return ;
}

void avistreamstop(int channel, char *data, int len)
{
#ifdef NETWORK_DEBUG
    DEBUG_PRI("avistreamstop packet receive...\n") ;
#endif
...
void avistreamreq(int channel, char *data, int len)
{
...
#endif
    int sendlen = 0, filecnt = 0, last_size = 0, stream_size, i = 0, j = 0, loop_flag = 0 ;
	int aviHdrSize = 0;
    long filesize ;
	...
	char dir_path[STRING_SIZE * 4] ;
	char file_name[STRING_SIZE * 4] ;
	...
	//# get full path of selcted file...
	sprintf(file_name, "%s.udf", strtok(Avistreamreq->filename, "."));
	sprintf(dir_path, "%s", app_file_get_dir_path(file_name, ON));
    ...
    else
    {
		...
		for(i = 0 ; i < filecnt; i++)
		{
			...
			//# Not used cause encoded each I-frame NAL unit, so impassible convert to avi.
			#if 0 
			if(i == 0)
			{
				//# read avi header and decoding...
				fread(&m_SendBuffer[channel][AVISTREAMDATA_SIZE], sizeof(AVIHeadBlock), 1, favi);
				aviHdrSize = sizeof(AVIHeadBlock);
				decodingHdr((AVIHeadBlock*)&m_SendBuffer[channel][AVISTREAMDATA_SIZE]);
					
			}
			#endif			
			
		    fread(&m_SendBuffer[channel][AVISTREAMDATA_SIZE+aviHdrSize], stream_size-aviHdrSize, 1, favi);
...
void loglistreq(int channel, char *data, int len)
{
#ifdef NETWORK_DEBUG
    DEBUG_PRI("loglistreq packet receive...\n") ;
#endif
    ...
    LOGLISTRES Loglistres ;

    dir_info = opendir(LOG_DIR_MMC) ;
    if( NULL != dir_info)
    {
        while((dir_entry = readdir(dir_info)) != NULL)
        {
            if(strstr(dir_entry->d_name, "log") != NULL)
            {
                printf("dir_entry->d_name = %s\n",dir_entry->d_name) ;

                sprintf(&Loglistres.file_name[cnt * STRING_SIZE], "%s",dir_entry->d_name);
                cnt += 1 ;
                printf("log count = %d\n",cnt) ;
            }
        }
    ...
    Loglistres.identifier = htons(IDENTIFIER) ;
    Loglistres.cmd        = htons(CMD_LOGLIST_RES) ;
    Loglistres.length     = htons(LOGLISTRES_SIZE + 2 + cnt*STRING_SIZE) ;
    if(cnt != 0)
        Loglistres.result     = htons(0x00) ;
    else
        Loglistres.result     = htons(0x01) ;
    Loglistres.file_count = htons(cnt) ;

    memcpy(m_SendBuffer[channel], &Loglistres, LOGLISTRES_SIZE + 2 + cnt*STRING_SIZE) ;
    sendlen = send(SystemInfo.Channel[channel], m_SendBuffer[channel], LOGLISTRES_SIZE + 2 + cnt*STRING_SIZE, 0) ;
#ifdef NETWORK_DEBUG
    DEBUG_PRI("loglistres packet sendlen = %d\n",sendlen) ;
#endif

}


void logfilereq(int channel, char *data, int len)
{
#ifdef NETWORK_DEBUG
    DEBUG_PRI("logfilereq packet receive...\n") ;
#endif
    long filesize = 0 ;
    int filecnt = 0, last_size = 0, stream_size = 0, i = 0, sendlen = 0 ;
    char file_name[128 ] ;
    char Temp_buffer[1024*1024] ;

    FILE *Fp = NULL ;

    LOGFILEREQ *Logfilereq ;
    LOGFILERES Logfileres ;
    COMRESPACKET LogfileErr ;

    Logfilereq = (LOGFILEREQ *)data ;

    sprintf(file_name, "%s/%s",LOG_DIR_MMC, Logfilereq->file_name) ;
    printf("file_name = %s\n",file_name) ;
    ...
    if(!Fp)
    { 
        LogfileErr.identifier = htons(IDENTIFIER) ;
        LogfileErr.cmd = htons(CMD_LOGFILE_ERR) ;    
        LogfileErr.length = htons(8) ;    
        LogfileErr.result = htons(0x01) ;

        memcpy(m_SendBuffer[channel], &LogfileErr, 8) ;
        sendlen = send(SystemInfo.Channel[channel], m_SendBuffer[channel], 8, 0) ;
#ifdef NETWORK_DEBUG
        DEBUG_PRI("logfile_err packet sendlen = %d\n",sendlen) ;
#endif    
    ...
    else
    {
        ...
        for(i = 0 ; i < filecnt; i++)
        {
            ...
            Logfileres.identifier    = htons(IDENTIFIER) ;
            Logfileres.cmd           = htons(CMD_LOGFILE_RES) ;
            Logfileres.length        = htons(10 + stream_size ) ;
            Logfileres.FragmentNo    = htons(filecnt) ;
            Logfileres.FragmentIdx   = htons(i) ;

            memcpy(m_SendBuffer[channel], &Logfileres, 10) ;
            fread(&m_SendBuffer[channel][10], stream_size, 1, Fp) ;

            sendlen = send(SystemInfo.Channel[channel], m_SendBuffer[channel], 10 + stream_size, 0);
#ifdef NETWORK_DEBUG
        DEBUG_PRI("logfile packet sendlen = %d\n",sendlen) ;
#endif
            usleep(5000) ;
        }
        fclose(Fp) ;
