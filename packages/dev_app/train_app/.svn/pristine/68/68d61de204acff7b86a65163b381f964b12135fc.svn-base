...
typedef enum {
	CH_TYPE_RECORDING=0,	//# recording channel	720p
	CH_TYPE_STREAMING,		//# streaming channel	640x480
...
typedef struct{
    char ipaddr[CHAR_16];
    char subnet[CHAR_16];
    char gateway[CHAR_16];
    char deviceID[CHAR_36];
	unsigned char reserved[128];
}app_server_t;
...
typedef struct {
	...
	app_watchdog_t	wd;
	app_network_t	net;				//# for ap-mode
	app_info_t		info;
    app_cam_bright_t cam;
	app_time_zone_t  tz;
	app_wifiap_t    wifiap;				//# for client-mode
    app_server_t    srv_info;
	app_sys_t		sys;
	unsigned char reserved[1048];	
}app_set_t;
