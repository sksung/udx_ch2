...
/*-----------------------------------------------------------------------*/
 ...
 #define SERDES_INI	"/mmc/serdes.ini"
 #define TITLE_EQ	"EQ_IDX(0~11):"
 #define TITLE_PE	"PE_IDX(0~12):"
 #define TITLE_IDX	"IDX(0~8):"
 #define RM_SERDES_INI "rm -f /mmc/serdes.ini"
 
#define MAX_SERDES_TEST_CNT	(9)
static serdes_val_t test_sample[MAX_SERDES_TEST_CNT] = {
	{1, 4}, {1, 6}, {1, 8}, {3, 4}, {3, 6}, {3, 8}, {5, 4}, {5, 6}, {5, 8} 
};

static int sample_idx = 0;

static void read_serdes_ini(int* eq, int* pe)
{
	FILE *fd;
	char title[2][CHAR_32];
	char val[2][CHAR_8];
	int i=0, next_idx = 0;

	if(-1 == access(SERDES_INI, 0))
	{
		fd = fopen(SERDES_INI, "wb+");
	}
	else {
		fd = fopen(SERDES_INI, "rb+");
		fscanf(fd,"%s %s", title[0], val[0]);

		sample_idx = atoi(val[0]);
	}

	printf("@@@@@@ READ %s - %d =======\n", SERDES_INI, sample_idx);
	
	*eq = test_sample[sample_idx].eq_idx;
	*pe = test_sample[sample_idx].pe_idx;

	next_idx = (sample_idx+1)%MAX_SERDES_TEST_CNT;
...
static void *get_g_mem_addr(int size, int *idx)
{
	...
	if(app_cfg->state.rec)		//# protect overflow
	{
...
//        if(app_cfg->state.rec)
		{
			...
			if(pFullBuf->codecType == IVIDEO_H264HP && pFullBuf->chnId < ENC_D1_CHANNEL)
			{
				...
				if(app_cfg->state.rtsptx)
				{
					...
					//# rtsp_tx	
					if(app_cfg->state.wifi && pFullBuf->codecType == IVIDEO_H264HP && ifr->ch == stream_ch){//app_cfg->en_dual){
...
/*
					...
					if(app_cfg->state.wifi && pFullBuf->codecType == IVIDEO_H264HP){
					    app_rtsptx_write((void *)ifr->buf, ifr->offset, ifr->b_size,
			   			       			ifr->is_key?FTYPE_VID_I:FTYPE_VID_P, ifr->ch, captime);
					}
*/
				}
#if 0
...
static void capt_enc_init(VENC_PARAMS_S *pContext)
{
	...
	int i, default_fps=DEFAULT_FPS;

	for(i=0; i<app_cfg->num_ch+app_cfg->en_dual; i++)
	{
		//# fps, bps set
		...
		//# enc motion disable
		pContext->encChannelParams[i].enableAnalyticinfo = 0;
		Venc_params_set(pContext, i, &params, VENC_ALL);

		if(app_cfg->en_jpg)
		{
			params.frameRate = JPEG_FPS;
			Venc_params_set(pContext, i+(app_cfg->num_ch+app_cfg->en_dual), &params, VENC_ALL);
		}		
	}    
}
...
static void capt_enc_late_init(void)
{
	...
	for(i=0; i<app_cfg->num_ch+app_cfg->en_dual; i++)
	{
		...
		if(app_cfg->en_jpg){
			cap_jpeg_quality(i+(app_cfg->num_ch+app_cfg->en_dual), JPEG_QUALITY);
		}
	}
}
...
static int capt_param_init(VCAP_PARAMS_S *vcapParams, VENC_PARAMS_S *vencParams)
{
	...
	for(i=0; i<app_cfg->num_ch+app_cfg->en_dual; i++)
	{
		...
		if(i == 1){
			app_cfg->ifrm[i].wi = 640;
			app_cfg->ifrm[i].he = 480;
	 		app_cfg->ifrm[i].fr = 10;
			br = 1000;	//# 1Mbps
    	    app_cfg->ifrm[i].br = (br * app_cfg->ifrm[i].fr)/default_fps;
 ...
 int app_capt_start()
{
	...
	//# check m3vpss device
	app_cfg->num_ch 	= 1;	//# Only used Cam1
	app_cfg->en_dual	= 1;	//# live streaming enable(640x480)
	app_cfg->layout		= UBX_LAYOUT_M;
	...
	vsysParams.enableDual		= app_cfg->en_dual;

	int eq, pe, eq_idx, pe_idx;
#ifdef FOR_CAM_TEST	
	read_serdes_ini(&eq_idx, &pe_idx);
#else
	get_serdes_lv(app_set->sys.serdes_level, &eq_idx, &pe_idx);
#endif

	eq = get_serdes_eq(eq_idx);
	pe = get_serdes_pe(pe_idx);
    vsysParams.serdesEQ[0]		= eq;
	vsysParams.serdesEQ[1]		= eq;
	vsysParams.serdesPE[0]		= pe;
	vsysParams.serdesPE[1]		= pe;

	sprintf(msg, " [APP_CAP] Init Serdes EQ: 0x%X, PE: 0x%X ...", eq, pe);
...
int cap_video_detect(void)
{
	...
	Vcap_get_video_status((Int32*)&video_detect, app_cfg->num_ch);
		
	if(video_detect[0] == 0){
		app_cfg->num_ch = 0;
		app_cfg->state.cam_buzzer = 1;

		sprintf( msg, "!!! Camera Is Not Detected !!!");
		app_log_write( MSG_LOG_WRITE, msg );		
	}
