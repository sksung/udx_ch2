...
{
	int     ret;
    UInt8   reg_val_thr = 99999;

	ret = Vcap_get_video_reg9272(ch, 0x10, &app_cfg->serdes_err_val);
//	ret = Vcap_get_video_reg9272(ch, 0x13, &reg_val_thr);

//	printf("%s err:%d ====\n", __func__, app_cfg->serdes_err_val);
...
//    dev = 1 - dev;	//CAM1 <-> CAM2(later ti_tools\hdvpss_01_00_01_37\packages\ti\psp\devices\ph3100k\src\vpsdrv_ph3100kI2c.c line 56, 57 change)
	...
	//# ph3100k: 10->high 1->low (min 64 ~ max 147)
	Vcap_set_video_reg(dev, 0x03, 0x04);
	if (value == 1) {
		Vcap_set_video_reg(dev, 0x3B, 0x4B); /* max_yt1 = 75 */
		Vcap_set_video_reg(dev, 0x3D, 0x4B); /* min_yt1 = 75 */
		Vcap_set_video_reg(dev, 0x3C, 0x40); /* max_yt2 = 64 */
		Vcap_set_video_reg(dev, 0x3E, 0x40); /* min_yt2 = 64 */
	} else if (value == 2) {
		Vcap_set_video_reg(dev, 0x3B, 0x63); /* max_yt1 = 99 */
		Vcap_set_video_reg(dev, 0x3D, 0x63); /* min_yt1 = 99 */
		Vcap_set_video_reg(dev, 0x3C, 0x58); /* max_yt2 = 88 */
		Vcap_set_video_reg(dev, 0x3E, 0x58); /* min_yt2 = 88 */
	} else if (value == 3) {
		Vcap_set_video_reg(dev, 0x3B, 0x73); /* max_yt1 = 115 */
		Vcap_set_video_reg(dev, 0x3D, 0x73); /* min_yt1 = 115 */
		Vcap_set_video_reg(dev, 0x3C, 0x68); /* max_yt2 = 104 */
		Vcap_set_video_reg(dev, 0x3E, 0x68); /* min_yt2 = 104 */
	} else if (value == 4) {
		Vcap_set_video_reg(dev, 0x3B, 0x83); /* max_yt1 = 131 */
		Vcap_set_video_reg(dev, 0x3D, 0x83); /* min_yt1 = 131 */
		Vcap_set_video_reg(dev, 0x3C, 0x78); /* max_yt2 = 120 */
		Vcap_set_video_reg(dev, 0x3E, 0x78); /* min_yt2 = 120 */
	} else if (value == 5) {
		Vcap_set_video_reg(dev, 0x3B, 0x93); /* max_yt1 = 147 */
		Vcap_set_video_reg(dev, 0x3D, 0x93); /* min_yt1 = 147 */
		Vcap_set_video_reg(dev, 0x3C, 0x88); /* max_yt2 = 136 */
		Vcap_set_video_reg(dev, 0x3E, 0x88); /* min_yt2 = 136 */
	}	
...
static int ctrl_set_fps(int mode)
{
	...
	{
		if(mode == CH_TYPE_RECORDING)
			app_rec_stop();

		ch = &app_set->ch[mode];

		fps = get_fps_val(ch->framerate);
		app_cfg->ifrm[mode].fr = fps;

		if(mode == CH_TYPE_RECORDING)
		{
			//#--- sensor framerate
			if(ctrl_hdcam_frame_set(mode, DEFAULT_FPS) != SOK)
			{
        ...
        //#--- set enc framerate
        if(ctrl_enc_fr_set(mode, fps) != SOK)
        {
        ...
        //#--- set enc i interval
        if(ctrl_enc_gop_set(mode, fps) != SOK)
        {
        ...
        app_cfg->ifrm[mode].br = (br * fps)/DEFAULT_FPS;

        if(ctrl_enc_br_set(mode, app_cfg->ifrm[mode].br) != SOK)
        {
        ...
        //#--- set qp
        if(ctrl_enc_rc_set(mode, ch->rate_ctrl, app_cfg->ifrm[mode].br) != SOK)
        {
...
frm_err:
	...
	if(!app_cfg->state.mmc_err && mode == CH_TYPE_RECORDING)
        app_rec_start();
...
int ctrl_set_enc_prm(int mode, int fps_idx, int bps_idx)
{
	...
	if(fps_idx > FPS_MAX || fps_idx < FPS_30 || bps_idx > MAX_QUALITY || bps_idx < Q_LV01)
		return EFAIL;

	ch = &app_set->ch[mode];
	bps_val = get_bitrate_val(bps_idx, ch->resol);

	if(bps_val > 0)
	{
		if(app_set->ch[mode].framerate != fps_idx)
		{
			app_set->ch[mode].framerate 	= fps_idx;
			app_set->ch[mode].quality 		= bps_idx;

			ret = ctrl_set_fps(mode);
		}
		else if (app_set->ch[mode].quality != bps_idx)
		{
			app_set->ch[mode].quality	= bps_idx;
...
int ctrl_time_set(int year, int mon, int day, int hour, int min, int sec, int utc)
{
	...
	if (utc) {
		...
		set = mktime(&ts) + ((h*3600)+(m*60)) + (get_summer_time_idx(app_set->tz.st_idx)*3600);	//# GPS sync with time_zone...
	}
	else {
		set = mktime(&ts);
	}
...
int ctrl_sd_format(void)
{
	...
	app_log_start();
	sleep(1);
	app_log_write(MSG_LOG_SHUTDOWN, ">>>>> SD card was formatted. System reset. <<<<<");

	app_file_sd_pwd_write();
...
void delete_nand_update()
{
    ...
    if(del==0){
	    ...
	    sprintf(cmd, "rm -f %s/%s", DIR_UPDATE, F_MICOM);
	    dev_execlp(cmd);
...
{
	static char fw_ver[CHAR_32] = {0,};

	#if CBR_ONLY
		sprintf(fw_ver, "%sC", TCX_SW_VER);
	#else
		sprintf(fw_ver, "%s", TCX_SW_VER);
	#endif
...
void app_ctrl_out_copy(void)
{
	...
	sprintf(path, "/mmc/%s_app.out", "train");

	if(0 == access(path, 0)) // existence only
	{
		sprintf(cmd, "cp %s /opt/%s/bin/.", path, "train");
		dev_execlp(cmd);
