...
typedef struct {
	...
	unsigned long	free_size;			//# storage free size (KB) - 1%
...
static char* rec_root[REC_STOR_MAX]={DIR_REC_MMC};
...
static int file_delete(unsigned long del_size_tot)
{
	if(ifile == NULL)
		return EFAIL;
	...
	if(ufs.fcnt > 0)
	{
		...
		while(del_size_tot > del_size_sum)
		{
			...
			if(remove(avi_path) != SOK)
			{
				eprintf("File remove fail!!");
				sprintf(cmd, "[APP_FILE] !!! File remove failed !!! - %s", avi_path);
				app_log_write( MSG_LOG_WRITE, cmd );
			...
			printf(" [File] Delete file: [%d]%s, size(del_tot,del_sum): (%ld, %ld)\n", 
					ufs.didx, avi_path, del_size_tot, del_size_sum);
...
static void* file_check_main(void* prm)
{
    ...
    while(!exit)
    {
		...
		if(app_cfg->state.mmc && ifile->root_idx < REC_STOR_MAX && !app_cfg->state.pwroff)
		{
			//# per 1 min
	        if(((f_cycle % FILE_LIST_CHECK_TIME) == 0) && app_cfg->state.mmc_err == 0)
	        {
				...
				printf("Check empty size / free size : %ld / %ld \n", idisk.avail, ifile->storage.free_size);
				if(idisk.avail < ifile->storage.free_size){
					file_delete(ifile->storage.free_size - idisk.avail);
				}
	        }
		}
...
//	app_log_write( MSG_LOG_WRITE, "[APP_FILE] Start file scanning...");
	...
	ifile->storage.sa_size = ((unsigned long long)idisk.total*DIR_PER)/100;	//# Used 99% size of disk total size.
	ifile->storage.free_size = idisk.total - ifile->storage.sa_size;		//# Set free size
//	ifile->storage.sa_size = idisk.total-FREE_SIZE; //# remain free size(100MB) for f/w, log, cfg files.
...
#endif
		sprintf(msg, ">>>>>>> Storage Status <<<<<<<");
		app_log_write( MSG_LOG_WRITE, msg );
		printf("%s \n", msg);

#if 0
		sprintf(msg, " DiskSize : %ld(KB)/%ld(KB)/%ld(KB)", ifile->storage.sa_size, idisk.total, ifile->storage.free_size);
		app_log_write( MSG_LOG_WRITE, msg );
		printf("%s \n", msg);
...
#if FREC_TEST
	ifile->storage.sa_size = (50*KB);	//# 50MB
#endif
		...
		for(d = 0; d < dir_idx; d++)
		{
			for(f = 0; f < ufs_info[d].fcnt; f++)
			{
				if(ufs_info[d].flist[f].del == 0 && ufs_info[d].flist[f].fsize > 0){
					...
					sprintf( (*flist)[frec_idx].fname, "%s", ufs_info[d].flist[f].fname);
...
#define SD_PWD_ITEM_CNT		2	
typedef struct{
    char item[5];
    char data[255];
}sd_pwd_t;

sd_pwd_t _pwd[SD_PWD_ITEM_CNT];
int sd_pwd_read_done = 0;

void app_file_sd_pwd_read(void)
{
	if(!app_cfg->state.mmc) return;

	FILE *fd_pwd;
	char file_path[128] = {0, };

	sprintf(file_path, "%s/%s", DIR_DISK_MMC, SD_PWD_FILE_NAME);
	if(-1 != access(file_path, 0))
	{
		fd_pwd = fopen(file_path, "r");

		if(fd_pwd != NULL)
		{
//			printf("@@@@ open %s ....\n", file_path);
			int	i = 0;
			while( i < SD_PWD_ITEM_CNT ){
				fscanf(fd_pwd,"%s %s", _pwd[i].item, _pwd[i].data);
				printf("%s %s \n", _pwd[i].item, _pwd[i].data);
				i++;
			}
...
void app_file_sd_pwd_write(void)
{
	if(!app_cfg->state.mmc) return;
	if(!sd_pwd_read_done) return;
