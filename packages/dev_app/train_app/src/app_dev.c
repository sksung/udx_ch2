...
static void app_dev_mmc_main(int notify)
{
	...
	if (notify)
	{
		...
		if(app_cfg->state.sd_insert) {
			app_cfg->state.mmc 			= 0; //# don't sync
			app_cfg->state.sd_insert 	= 0;
		...
		if (!app_cfg->state.format && !app_cfg->state.sd_insert) {
			sprintf( msg, " !!! SD card INSERT !!!");
			app_log_write( MSG_LOG_WRITE, msg );
			dprintf("SD card INSERT!!!\n");
...
static void *dev_main(void *prm)
{
    ...
    if(ts.tm_year < 117){
        ctrl_time_set(2017, 1, 1, 0, 0, 0, 0); // time default when the year under 2017...
    }
...
static int prev_serdes_err_val = 50000;

#ifdef FOR_CAM_TEST
	#define CAM_TEST_TIMEOUT_SEC 	(60)
	#define REBOOT_3SEC (3)
	static int timer_cnt = 0;
	static int cam_test_reboot = 0;
	static int reboot_timer_cnt = 0;
	static int error_val_total = 0;
	static int error_val_everage = 0;
#endif
...
static void *timer_main(void *prm)
{
	...
	int exit = 0;
	int rec_led = 0;
	int val, mmc_fd, res = 0, wcli_start = 0;
	char msg[CHAR_128]={0,};
	...
	while (!exit)
	{
		...
		//#--- 60sec deley start because sylix ap start is slower.
		//------------------------------------//
		if(wcli_start == 0)
		{
		...
		//#--- Serdes Error check...
		//------------------------------------//
		if( (app_cfg->state.cap) && (timer_cycle%DEV_1_SEC == 0) )
		{
			ctrl_hdcam_err_state(0);
			
			//////////# for test ////////////////		
//			app_cfg->serdes_err_val = rand()%10;
//			printf("#### serdes val >>>>>>> %d \n", app_cfg->serdes_err_val);
			////////////////////////////////////
			
#ifdef FOR_CAM_TEST
			timer_cnt++;
			error_val_everage = (error_val_total += app_cfg->serdes_err_val) / timer_cnt; 
//			printf("#### error val everage >>>>>>> %d \n", error_val_everage);
			if(timer_cnt == CAM_TEST_TIMEOUT_SEC)
			{
				int eq = 0, pe = 0;
				app_cap_test_idx(&eq, &pe);
			
				timer_cnt = 0;
				if(error_val_everage > 0) {
					sprintf(msg, " >>> SERDES ERROR !!! - EQ_IDX[%02d] PE_IDX[%02d] <<< ", eq, pe);
				}
				else {
					sprintf(msg, " >>> SERDES OK !!! - EQ_IDX[%02d] PE_IDX[%02d] <<< ", eq, pe);
				}
			...
			if(cam_test_reboot) {
				reboot_timer_cnt++;
				if(reboot_timer_cnt > REBOOT_3SEC) {
					reboot_timer_cnt = 0;
					dev_system_reboot(REBOOT_NORMAL);
				}
			}
#else
			if( (prev_serdes_err_val != app_cfg->serdes_err_val) && (app_cfg->serdes_err_val > SERDES_ERR_MIN_CNT) )
			{
				prev_serdes_err_val = app_cfg->serdes_err_val;
				sprintf(msg, " >>> SERDES ERROR !!! <<< ");
				printf("%s \n", msg);
				app_log_write(MSG_LOG_WRITE, msg);
			}
#endif			
		...
		//#--- led display
		//------------------------------------//
		if((timer_cycle != 0) && (timer_cycle%(TIMER_INTERVAL*3) == 0))
		{
			if(app_cfg->state.cap){
				//# ACT LED State ac led on when start recording...
				...
				//# REC LED State
				//# Record fail....
				if(app_cfg->state.mmc == 0 || app_cfg->state.mmc_err){
					rec_led = !rec_led;
					rec_led_run(rec_led);
				}
				//# Others...
				else if(app_cfg->state.cam_buzzer || app_cfg->state.wifi_usb == 0){
//					printf("REC_LED_ON1 = cam_buzzer: %d wifi_usb: %d ========\n", 
//						app_cfg->state.cam_buzzer ,app_cfg->state.wifi_usb);
					rec_led_run(ON);
				}
				//# No errors...
				else{
...
int system_buzzer(int state){
    ...
    if(state == SYSTEM_OPER){
        ...
        if(app_set->aud.snd_on)
            mic_send_msg(CMD_BUZZER, 2, BUZZER_100);
//        printf("REC_LED_ON2 = system_buzzer() SYSTEM_ERR!!! ========\n");
        rec_led_run(ON);
    }
