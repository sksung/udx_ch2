...
#ifdef USE_5V
#define TIME_PEEK_VOLT_CHK	 400	//# peek low voltage check time
#else
...
#ifdef USE_5V
#define START_VOLTAGE_5V	450	//# 4.5V
#endif
...
static int dev_check_volt(int volt)
{
	...
	if(volt < VOLT_LOW)
	{
        ...
        if(volt_peek_cnt >= CNT_PEEK_VOLT_CHK && !imicom->low_pwr_detect)
        {
			...
			printf(" \n[app] <<<<<<< Peek Low Voltage Detected!!!(%d.%dV) >>>>>>> \n", (volt/100), (volt%100));
...
#ifdef FOR_CAM_TEST
			app_cap_del_serdes_ini();
#endif

//            if(app_cfg->volt_level == VOLT_LEVEL_12V)
    		    mic_exit_state(OFF_LOWPWR, VOLT_LOW);
...
//            system_buzzer(SYSTEM_OPER);
    		...
    		acc_cnt 	= 0;			
            sprintf(msg, "[APP_MICOM] ACC OFF power off!!");
		    app_log_write(MSG_LOG_SHUTDOWN, msg);
	...
	while(!exit)
	{
		...
		switch(msg.cmd)
		{
			case CMD_DEV_VAL:
			{
				...
				//# It is used to 5 minute recording after acc off for DTC_DRIVE
				//# --- check acc -----------------
...
int app_micom_init(void)
{
    ...
    sprintf(app_cfg->micom_ver, "v%02x.%02x", (mic_ver>>8) & 0xFF, mic_ver & 0xFF);

	app_watchdog_init(ON, TIME_WATCHDOG);
//	app_watchdog_init(OFF, TIME_WATCHDOG);
