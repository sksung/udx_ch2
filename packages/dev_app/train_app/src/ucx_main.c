...
void time_debug_clock(char *func, int type)
{
    ...
    struct tm date;
    time(&the_time);
    localtime_r((const time_t *)&the_time, &date);
...
int main(int argc, char **argv)
{
	...
	if (!ctrl_mmc_check_partitions()) {
		...
		if (ret)
		{
			sprintf(msg, "mmc detected recoverable errors(ret %d)!\n", ret);
...
//				printf("mmc errors repaired!\n");
	...
	app_cfg->state.sd_insert = app_cfg->state.mmc;
	
    if((sd_res >= app_cfg->state.mmc) || (!app_cfg->state.mmc)){
        printf("state.mmc_err = 1 \n");
        app_cfg->state.mmc_err = 1;
        app_cfg->state.sd_buzzer = 1;
		app_cfg->state.sd_insert = 1;	//sd inserted, but not mounted because some errros.
    }
	...
	sprintf(msg, "HW_Ver: %s, SW_Ver: %s, MICOM_Ver: %s",app_set->info.hw_ver, ctrl_get_fw_ver(), app_cfg->micom_ver);
	app_log_write(MSG_LOG_WRITE, msg);
...
//  app_aud_capt_ctrl(app_set->aud.mic_on);	// not use mic in dtc-train

	//#--- main --------------
	app_main();
	//#-----------------------

	app_log_write( MSG_LOG_WRITE, "[UCX_MAIN] app_main() EXIT...." );
