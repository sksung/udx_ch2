...
//#define EN_WIFI_CLIENT	//# WIFI Client mode
//#define FOR_CAM_TEST	//# Camera sync error check with eq and pe values 
...
//#------- F/W Version -------//
		...
		#define TCX_SW_VER		"3.09.00C"
	#else
		#define TCX_SW_VER     	"3.09.00"
	#endif
	
#else

	#ifdef EN_WIFI_CLIENT
		#define TCX_SW_VER		"3.09.01C"
	#else
		#define TCX_SW_VER		"3.09.01"
	#endif
...
#define TVO_VOLT		(1<<2)
#define TVO_MIC         (1<<3)
#define TVO_BUZZER		(1<<4)
#define TVO_VERSION		(1<<5)
...
typedef	struct {
    ...
    unsigned int wifi_usb   :1; //# for wifi usb detect.
	unsigned int sd_insert	:1;	//# sd insert/remove
} app_state; //# 30bits
...
typedef struct {
    ...
    int serdes_err_val;	//# serdes err register value;
} app_cfg_t;
